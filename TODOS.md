...
- [ ] **v0.37.x: Live-test non-Anthropic OpenRouter families (OpenAI / Gemini) for the subagent loop.** Same abort/retry pin as Anthropic-via-OR before flipping the predicate wider. DeepSeek DONE: `openrouter:deepseek/…` declares `supports_subagent_loop` and auto-routes through the gateway loop like Anthropic-via-OR; pin at `test/e2e/openrouter-deepseek-subagent-replay.live.test.ts`. Family list lives in `src/core/ai/openrouter-families.ts`.
