...
- [ ] **v0.36.x: Supabase parity test for `applyForwardReferenceBootstrap`.** Codex C6 residual coverage gap from the v0.36 bootstrap wave. The bootstrap probes use `this.sql` (instance pool) but `PostgresEngine.initSchema` chooses a DDL connection internally. The new pre-v18/pre-v34/pre-v60 E2E tests (`test/e2e/postgres-bootstrap.test.ts` and `test/schema-bootstrap-coverage.test.ts`) run against local Docker Postgres — they catch schema correctness but NOT Supabase pooler/direct-pool routing or statement_timeout interactions. Real Supabase upgrades have failed multiple times on this exact connection-topology divergence (#699, #820 lineage). Fix: a test fixture that exercises the probe path against the Supabase-shape connection topology (deriveDirectUrl + transaction pooler + statement_timeout). Pros: catches a regression class Docker can't see. Cons: requires Supabase fixture infra OR careful mocking of the connection-selection logic in `db.ts`'s `getDDLConnection` path. Plan reference: `~/.claude/plans/ok-i-spun-up-dreamy-thompson.md` "Supabase pooler/direct-pool parity testing" out-of-scope item.
