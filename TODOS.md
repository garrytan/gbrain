...
## kinshasa-v3 follow-ups (v0.35.2.0)

- [ ] **v0.36.x: Fix `supervisor-audit.ts:77` `readSupervisorEvents` to use the dual-week-aware pattern from `stub-guard-audit.ts:readRecentStubGuardEvents`.** The supervisor reader only reads the current ISO-week file, so a 24h sliding window across Monday 00:00 UTC silently loses Sunday's events (they're in last week's file). The new stub-guard reader in v0.35.2.0 fixes this for its own audit log by reading BOTH current and previous week files before timestamp-filtering — the supervisor reader should adopt the same shape. Pin with a unit test that uses a fake-clock fixture set to "Monday 00:01 UTC" with a Sunday 23:55 event in the prior file. Filed during v0.35.2.0 kinshasa-v3 codex outside-voice review.
...
- [ ] **v0.36.x: `PREFIX_EXPANSION_DIRS` is hardcoded to `['people', 'companies']` in `src/core/entities/resolve.ts:97`.** New entity directories (funds, advisors, deals, etc.) require a code change to opt in. Consider a config-driven list (`entities.prefix_expansion_dirs: [...]` in `gbrain.yml`) so operators can extend without forking. Filed during v0.35.2.0 plan-eng-review.

- [ ] **v0.36.x: Sweep `wintermute` references out of `CHANGELOG.md`.** Two pre-existing lines in `CHANGELOG.md` reference the private agent name in narrative text — pre-existing on master, not introduced by v0.35.2.0. Per CLAUDE.md's "never say Wintermute in public artifacts" rule, these should be replaced with `your OpenClaw` or `Garry's OpenClaw`. Trivial sed-style cleanup; do it in the next housekeeping PR. Filed during v0.35.2.0 privacy audit.
