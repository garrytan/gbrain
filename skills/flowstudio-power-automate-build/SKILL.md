...
|---|---|---|
| **Chat with Flow bot** | Plain email string with **trailing semicolon** | `"email-redacted.example;"` |
| **Channel** | Object with `groupId` and `channelId` | `{"groupId": "...", "channelId": "..."}` |

> **Common mistake**: passing `{"to": "email-redacted.example"}` for "Chat with Flow bot"
...
- Implements the upstream GitHub awesome-copilot skill intent for `flowstudio-power-automate-build`: >-
