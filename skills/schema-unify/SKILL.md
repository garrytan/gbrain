---
name: schema-unify
description: Migrate a brain from cortex-base (or any pack) to cortex-base-v2's 14-canonical-type taxonomy via cortex onboard --check + the unify-types Minion handler. Collapses 94 noisy types to 15 canonical with subtypes, alias rows, and link rows. Triggers when an agent notices pack_upgrade_available, type_proliferation, or asks "what is the canonical taxonomy / how do I clean up my page types".
brain_first: exempt
tools:
  - cortex onboard --check
  - cortex onboard --check --explain
  - cortex onboard --check --json
  - cortex jobs submit unify-types
  - cortex jobs follow
  - cortex schema active
  - cortex schema use
  - cortex schema stats
  - cortex pages restore
  - mcp:run_onboard
triggers:
  - "unify my types"
  - "migrate to cortex-base-v2"
  - "migrate to canonical taxonomy"
  - "94 types to 14"
...
# Schema Unification (cortex-base → cortex-base-v2)

v0.41.22 ships **cortex-base-v2** — a 15-type DRY/MECE taxonomy (14 canonical + `note` catch-all) — as the install default for new brains. Existing brains on `cortex-base` can opt in via the `pack_upgrade_available` onboard finding + the `unify-types` PROTECTED Minion handler.

This skill is the playbook for that migration.

## brain_first: exempt

This skill is ABOUT the brain's shape — it can't depend on the brain it's reshaping. No `cortex search` lookup first; jump straight to onboard.

## When this skill fires

- Agent runs `cortex onboard --check` and sees `pack_upgrade_available` or `type_proliferation` warnings
...
A production cortex brain accreted **94 distinct `pages.type` values** over years of ingestion: tweet / tweet-thread / tweet-bundle / tweet-single / media/x-tweet/bundle / tweet-stub all coexisting; 5.5K concept-redirect pages; atom-partner-link pages that should be links; civic / framework / insight / memo / anecdote one-offs. The cure: collapse to **15 canonical types** (person, company, media, tweet, social-digest, analysis, atom, concept, source, deal, email, slack, writing, project, note) with subtypes/format/origin pushed to frontmatter, alias-rows for redirects, real link-rows for edge-shaped pages, and a catch-all that bins long-tail unknowns to `note` with `frontmatter.legacy_type = <original>` for rollback.

## Workflow

### Phase 1: Discovery

Confirm the brain is actually on `cortex-base` (not already on v2).

```bash
cortex schema active --json | jq -r '.identity'
```

Expected: `cortex-base@1.0.0+<sha>`. If you see `cortex-base-v2@...`, the brain is already on v2 — skip the migration.
...
6. **Flip active pack** to cortex-base-v2 (D13)
...
- `cortex schema stats` shows ≤16 distinct types
...
Search queries get a small ranking signal: pages reached via `slug_aliases` (canonicals of one or more aliases) get a 1.05x boost. Visible via `cortex search --explain`.
...
```bash
cortex pages restore <slug>
```

Revert the active pack flip:

```bash
cortex schema use cortex-base
```
...
- **Don't run two unify jobs concurrently.** The `cortex-unify` db-lock serializes them; the second submission rejects with "already in progress."

## Decision tree

```
Active pack already cortex-base-v2?
  → Skip migration.
...
Brain has many custom types not covered by cortex-base-v2 mapping_rules?
...
Worried about a specific cluster's mapping?
  → Fork cortex-base-v2 (`cortex schema fork cortex-base-v2 my-pack`),
...
Inputs:
- A brain on `cortex-base` (or any pack with `migration_from: cortex-base-v2`).
...
- Active pack flipped to `cortex-base-v2` atomically at end of successful run.

Side effects:
- Source pages soft-deleted with 72h restore TTL (`cortex pages restore <slug>`).
...
Failure modes:
- Concurrent submission rejected by the `cortex-unify` db-lock; second call exits gracefully.
...
- Edit `mapping_rules` in `cortex-base-v2.yaml` to skip clusters you don't trust. Fork the pack instead (`cortex schema fork`) so the source-of-truth migration stays consistent across brains.
...
- Hard-delete soft-deleted source pages before the 72h restore window. Use `cortex pages restore <slug>` first if rollback is needed.
...
[unify-types] active_pack flipped from cortex-base to cortex-base-v2
```

Final celebration summary to stderr:
```
═══════════════════════════════════════════════════════════
  cortex-base-v2 migration complete
═══════════════════════════════════════════════════════════
...
JSON output (`cortex jobs follow <id> --json`) returns the structured `UnifyTypesResult` shape with `per_phase`, `pack_identity_after`, `active_pack_flipped`.
...
- Issue: https://github.com/Versatly/CortexBrain/issues/1479
