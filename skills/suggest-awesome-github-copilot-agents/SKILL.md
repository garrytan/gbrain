...
- Implements the upstream GitHub awesome-copilot skill intent for `suggest-awesome-github-copilot-agents`: Suggest relevant GitHub Copilot Custom Agents files from the awesome-copilot repository based on current repository context and chat history, avoiding duplicates with existing custom agents in this repository, and identi
