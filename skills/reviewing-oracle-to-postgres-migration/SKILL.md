...
triggers:
  ...
  - "reviewing oracle to postgres migration"
  - "awesome copilot reviewing oracle to postgres migration"
---
...
- Implements the upstream GitHub awesome-copilot skill intent for `reviewing-oracle-to-postgres-migration`: Identifies Oracle-to-PostgreSQL migration risks by cross-referencing code against known behavioral differences (empty strings, refcursors, type coercion, sorting, timestamps, concurrent transactions, etc.). Use when plan
