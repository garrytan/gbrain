...
triggers:
  - "pester-migration"
  - "pester migration"
  - "awesome copilot pester migration"
---
...
- Implements the upstream GitHub awesome-copilot skill intent for `pester-migration`: Experimental (preview) Pester migration skill for upgrading PowerShell Pester test suites across major versions — v3→v4, v4→v5, and v5→v6. The v5→v6 path tracks Pester 6, which is still a release candidate, so that guida
