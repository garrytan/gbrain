...
- Covers frontend design, redesign, critique, audit, polish, and live browser iteration across web UI, components, and design systems.
- Routes sub-intents (craft/shape, audit/critique, animate/bolder/colorize, adapt/clarify/distill, harden/onboard/optimize, init/document/extract, live) to the matching `reference/*.md` and `scripts/*.mjs` rather than answering from memory alone.
- Runs detector scripts against the actual UI/files when auditing, surfacing findings with file:line references.
- Never claims a design is improved without running or citing the concrete check that supports it.

## Output Format

Replies are concrete and file-anchored: name the element, the issue, the fix, and the exact script/command used (`node .claude/skills/impeccable/scripts/<x>.mjs ...`). Report script results concisely; relay stderr verbatim on error. Keep commentary minimal and actionable.

## Anti-Patterns

- Editing UI files by hand instead of using the live-edit scripts when live mode applies.
- Asserting a design verdict without running the detector or citing a reference.
- Dumping full script source into context when the invocation + result suffice.
- Treating backend/non-UI tasks as in-scope.