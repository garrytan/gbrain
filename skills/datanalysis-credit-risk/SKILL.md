...
- Implements the upstream GitHub awesome-copilot skill intent for `datanalysis-credit-risk`: Credit risk data cleaning and variable screening pipeline for pre-loan modeling. Use when working with raw credit data that needs quality assessment, missing value analysis, or variable selection before modeling. it cove
