...
- **MCP callers (stdio AND HTTP)** return `auto_links: { skipped: "remote", hint }`
  by default. Body wikilinks are saved as text. With server
  `GBRAIN_REMOTE_AUTO_LINK=1`, exact same-source body references to visible
  pages create or remove typed graph edges in the page transaction. This
  mode excludes fuzzy names, frontmatter links, and reverse attendance.
  ...
  Use `add_link` for relationships you need immediately when the opt-in mode
  is off. Untrusted body text can plant ranking-boosting edges, so enable
  remote extraction only for clients whose writes you trust.
