...
triggers:
  ...
  - "sql server table reconciliation"
  - "awesome copilot sql server table reconciliation"
---
...
- Implements the upstream GitHub awesome-copilot skill intent for `sql-server-table-reconciliation`: Use when: comparing SQL Server tables across instances, data migration validation, ETL verification, row mismatch detection, schema drift, reconciliation report, production vs staging comparison. Uses mssql-python driver
