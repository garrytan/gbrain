...
- Implements the upstream GitHub awesome-copilot skill intent for `transloadit-media-processing`: Process media files (video, audio, images, documents) using Transloadit. Use when asked to encode video to HLS/MP4, generate thumbnails, resize or watermark images, extract audio, concatenate clips, add subtitles, OCR do
