...
# EIIRP files across the full canonical set. The actual destination per
# page is decided by brain-taxonomist consulting the active schema pack via
# `gbrain schema show --json`; frontmatter lists the gbrain-recommended set
# so the filing-audit gate passes.
