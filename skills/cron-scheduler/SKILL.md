...
When the brain has 2+ active sources (anything `cortex sources list` shows
...
```cron
*/5 * * * * cortex sync --all --parallel 4 --workers 4 --skip-failed
```
...
**Avoid (legacy)**: separate `cortex sync --source default` and
`cortex sync --source zion-brain` entries staggered by 5 minutes. They
require manual deconfliction every time a new source is added, and a
slow source can race a fast source on the legacy global `cortex-sync`
lock (v0.40.3.0+ uses per-source `cortex-sync:<sourceId>` locks but the
per-source cron pattern doesn't benefit from the parallelism that
`--all --parallel` actually delivers).

`cortex doctor` surfaces the recommended line as a `sync_consolidation`
...
- Separate per-source `cortex sync --source <id>` cron entries when
  `cortex sync --all --parallel N --workers N` would replace them with
