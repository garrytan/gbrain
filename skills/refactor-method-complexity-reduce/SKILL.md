...
- Implements the upstream GitHub awesome-copilot skill intent for `refactor-method-complexity-reduce`: Refactor given method `${input:methodName}` to reduce its cognitive complexity to `${input:complexityThreshold}` or below, by extracting helper methods.
