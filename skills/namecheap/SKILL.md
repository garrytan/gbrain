...
python3 namecheap.py domains.dns.setEmailForwarding --domain example.com --mailbox info --forward-to email-redacted.example
...
```json
[
  { "MailBox": "info", "ForwardTo": "email-redacted.example" },
  { "MailBox": "sales", "ForwardTo": "email-redacted.example" }
]
```
...
- Implements the upstream GitHub awesome-copilot skill intent for `namecheap`: Manage DNS records for domains registered with Namecheap via their API. List domains, view/add/update/remove DNS host entries (A, AAAA, CNAME, MX, TXT, etc.), and guide users through API setup including public IP detecti
