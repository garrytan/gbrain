...
triggers:
  - "dotnet-mcp-builder"
  - "dotnet mcp builder"
  - "awesome copilot dotnet mcp builder"
---
...
- Implements the upstream GitHub awesome-copilot skill intent for `dotnet-mcp-builder`: Build Model Context Protocol (MCP) servers in C#/.NET against the current ModelContextProtocol 1.x NuGet packages. Especially helps with cases the model often gets wrong without guidance — stale preview versions (it tend
