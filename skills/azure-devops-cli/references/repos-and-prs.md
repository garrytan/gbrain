...
az repos pr create \
  ...
  --draft true \
  --reviewers email-redacted.example email-redacted.example \
  --required-reviewers email-redacted.example \
...
```bash
# Add reviewers
az repos pr reviewer add --id {pr-id} --reviewers email-redacted.example email-redacted.example

# List reviewers
az repos pr reviewer list --id {pr-id}

# Remove reviewers
az repos pr reviewer remove --id {pr-id} --reviewers email-redacted.example
```
