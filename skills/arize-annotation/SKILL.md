...
triggers:
  - "arize-annotation"
  - "arize annotation"
  - "awesome copilot arize annotation"
---
...
ax annotation-queues create \
  ...
  --annotator-email email-redacted.example \
...
annotations_df = pd.DataFrame([
    {
        ...
        "annotation.Correctness.updated_by": "email-redacted.example",
    },
    {
...
- Implements the upstream GitHub awesome-copilot skill intent for `arize-annotation`: Creates and manages annotation configs (categorical, continuous, freeform label schemas) and annotation queues (human review workflows) on Arize. Applies human annotations to project spans via the Python SDK. Use when th
