...
feature_pitch:
  ...
  description: |
    Cortex operators: your gateway pins at 100% CPU because most cron jobs
    ...
    scale. Retry, backoff, DLQ, unified `cortex jobs list` visibility, all
...
**Audience: host agents (Cortex deployments, Hermes operators, future
hosts) reading this AFTER `cortex apply-migrations` has run. There is no
...
**Postgres** (most Cortex/Hermes deployments):
...
|---------|-------|--------|
| `agentTurn <skill>` or any Cortex-dispatched LLM skill | LLM-requiring | **Leave as-is.** Needs gateway. |
...
**Before** (LLM gateway):
```
Cortex cron: x-cortex-unified, 3 13,16,19,22,1,4,7,10 * * *
  → runs agentTurn x-cortex-unified
...
3 13,16,19,22,1,4,7,10 * * * \
  cortex jobs submit shell \
    --params '{"cmd":"node /data/.cortex/workspace/scripts/x-cortex-daily.mjs","cwd":"/data/.cortex/workspace"}' \
...
```bash
cortex jobs submit shell \
  --params '{"cmd":"node scripts/yc-sync.mjs","cwd":"/data/.cortex/workspace","env":{"YC_API_TOKEN":"'"$YC_API_TOKEN"'"}}'
```
...
1. Wait for the next scheduled fire (or trigger manually: `cortex jobs
   submit shell --params '...' --follow`).
2. Check `cortex jobs list --status completed --name shell --limit 5`
   for the result.
3. `cortex jobs get <id>` shows `exit_code`, `stdout_tail`, `stderr_tail`,
   `duration_ms`.
...
`CORTEX_ALLOW_SHELL_JOBS=1` on the worker, jobs sit in `waiting`
...
- **Automatic crontab rewrites.** Deferred to a future `cortex
...
- **Orphaned-shell-job stats.** `cortex jobs stats --orphaned` would
...
1. The worker runs with `CORTEX_ALLOW_SHELL_JOBS=1` (Postgres) or every
...
4. `cortex jobs stats` shows shell jobs completing at expected rates
