...
uses a hosted server plus an HTTPS tunnel. Simpler, more reliable, works with
any AI client.
...
```bash
ls ~/.cortex/integrations/twilio-voice-brain/heartbeat.jsonl 2>/dev/null
```
...
- Is there a public URL configured in `~/.cortex/config.json`?
- Is the OS Linux (likely server) or macOS (likely local)?

**Server** (Cortex/Hermes on Render/Railway): use 2-tier flow (WebRTC remote, then phone)
...
with `cortex integrations show twilio-voice-brain`." Skip to Step 6.
...
`cortex integrations show twilio-voice-brain`."
...
- Remote MCP now uses a hosted server plus an HTTPS tunnel
...
echo '{"version":"0.8.0","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","voice_setup":"RESULT"}' >> ~/.cortex/migrations/completed.jsonl
```

Tell the user: "v0.8.0 migration complete. Run `cortex integrations doctor` to
