...
1. Schema-pack additions: `conversation` (temporal, extractable) and `atom` (annotation, NOT extractable) promoted into `cortex-base`.
2. New CLI command `cortex extract-conversation-facts` for bulk fact extraction over long-form conversation pages.
...
`cortex upgrade` + `cortex apply-migrations --yes` handles the binary + schema. The cycle phase and doctor check require no agent action.
...
1. **Estimate cost first.** For each source the user wants to backfill:
   ```bash
   cortex sources audit <source-id> --json | jq '.facts_backfill_estimate'
   ```
...
2. **Run a dry-run** to preview segmentation:
   ```bash
   cortex extract-conversation-facts --dry-run --slug <slug-of-one-conversation-page>
   ```
...
3. **Run with explicit cost cap.** Default cap is $5. Recommend matching the audit estimate (rounded up):
   ```bash
   cortex extract-conversation-facts --background --max-cost-usd <cap>
   cortex jobs follow <printed-job-id>
   ```
...
4. **Verify with doctor:**
   ```bash
   cortex doctor --json | jq '.checks[] | select(.name == "conversation_facts_backlog")'
   ```
...
5. **(Optional) Enable autopilot drain** if the user has steady conversation ingest:
   ```bash
   cortex config set cycle.conversation_facts_backfill.enabled true
   ```
...
- Extracted facts use `source = 'cli:extract-conversation-facts'`. To remove them in bulk (rare), the only path today is raw SQL: `DELETE FROM facts WHERE source LIKE 'cli:extract-conversation-facts%'`. A `cortex forget --where` bulk flag is a v0.42+ TODO.
