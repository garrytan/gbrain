...
- **`cortex takes`** CLI — list / search / add / update / supersede / resolve.
...
- **`cortex think` op surface** — registered now; pipeline lands in v0.28.x.

## What the migration did automatically

`cortex apply-migrations --yes` runs three orchestrator phases:
...
2. **Backfill takes** — walked every page in the brain, parsed any fenced
   `<!--- cortex:takes:begin/end -->` table found, populated the takes
   index in Postgres. Idempotent; safe to re-run.
3. **Re-chunk TODO queued** — `~/.cortex/migrations/pending-host-work.jsonl`
...
cortex doctor --check takes_fence_chunk_leak
...
cortex extract takes --rebuild
cortex sync   # picks up the markdown delta if any
```
...
```bash
# Old (deprecated):
cortex config set dream.synthesize.model claude-sonnet-4-6
cortex config set dream.patterns.model claude-sonnet-4-6

# New (one key controls everything):
cortex config set models.default sonnet

# Per-op override (optional):
cortex config set models.dream.synthesize opus

# Cleanup deprecated keys:
cortex config unset dream.synthesize.model
cortex config unset dream.patterns.model
```
...
(e.g., your own Cortex deployment) need explicit broader visibility:
...
cortex auth permissions <token-name> set-takes-holders world,garry,brain

# Or create a new token with the wider set up-front
cortex auth create my-claude-desktop --takes-holders world,garry,brain
```

Tokens for third-party agents (claude.ai, public integrations) should
keep the default `["world"]` — Cortex's hunches stay private.

### 5. Try the takes layer

```bash
# Add a take by hand
cortex takes add people/alice-example \
  --claim "Strong technical founder I have ever met" \
  --kind take --who garry --weight 0.85 \
  --source "OH 2026-05-01"

# List
cortex takes people/alice-example

# Search
cortex takes search "technical founder"

# Resolve a bet
cortex takes resolve people/alice-example --row 3 --outcome true \
...
```bash
cortex doctor         # all checks green; schema_version >= 32
cortex stats          # numbers stable

# If any step failed, file an issue:
# https://github.com/Versatly/CortexBrain/issues
# Include: cortex doctor output, ~/.cortex/upgrade-errors.jsonl if it exists,
...
- `cortex think` synthesis pipeline (gather → RRF → cite → synthesize)
- `cortex takes seed <slug>` — LLM extracts claims from page prose
