...
**TL;DR:** this release teaches the knowledge graph to read your YAML frontmatter. Every `company:`, `investors:`, `attendees:`, `key_people:`, `partner:`, `lead:`, and `related:` field you already wrote now surfaces as a typed graph edge. `cortex graph <hub-entity> --depth 2` goes from returning ~7 nodes to 50+ on a real brain without you changing a word of content.

For most users: run `cortex upgrade` and you're done. The orchestrator handles schema + backfill in 2-5 minutes on a 46K-page brain. You immediately see richer results from `cortex graph` queries.
...
The migration is resumable. If it dies mid-backfill (OOM, network blip), re-run `cortex upgrade` and it picks up where it left off via `ON CONFLICT DO NOTHING` on the new unique constraint.
...
# Sample a hub entity from your Cortex workspace — depth-2 should return many more nodes than before.
cortex graph <hub-entity-slug> --depth 2

# Filter to specific edge types (new in v0.13).
cortex graph <hub-entity-slug> --depth 2 --type yc_partner,invested_in

# Count edges by provenance.
cortex call get_stats --json
```

## Troubleshooting

**Migration failed mid-backfill.** Re-run `cortex upgrade`. Resumable via ON CONFLICT DO NOTHING + origin_page_id scoping.
...
- Create the missing pages (then run `cortex extract links --source db --include-frontmatter` to backfill).
...
**`attendee` vs `attended` type normalization.** Legacy rows with `link_type='attendee'` or `link_type='mention'` keep working. Normalization to the v0.13 canonical names (`attended`, `mentions`) is deliberately NOT in this migration — it's a separate semantic concern. The `cortex normalize-types` command (v0.14) handles it opt-in.

## For downstream agent skill forks

If you maintain a fork of Cortex skills (a custom Cortex deployment or agent-fork), check `docs/UPGRADING_DOWNSTREAM_AGENTS.md` for the v0.13 section. Verdict: **no action required for most skills.** Three skills (`meeting-ingestion`, `enrich`, `idea-ingest`) get a new optional phase if you want to consume the `auto_links.unresolved` field.

## If something goes wrong

1. `cortex doctor` — surfaces any partial migrations and any post-upgrade failures recorded in `~/.cortex/upgrade-errors.jsonl`.
2. Paste the recovery hint doctor prints.
3. If that fails too, file an issue: https://github.com/Versatly/CortexBrain/issues with doctor output + upgrade-errors.jsonl contents. This is how the cortex maintainers find fragile upgrade paths.
