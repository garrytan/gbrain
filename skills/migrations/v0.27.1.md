...
date: 2026-05-05
feature_pitch: |
  cortex remembers what you SAW, not just what you typed. Drop a screenshot,
  whiteboard photo, or iPhone HEIC into your Cortex workspace repo, run sync, and the
...
`cortex upgrade` runs `cortex apply-migrations --yes` automatically, which
...
- **Off by default**: nothing changes at runtime until you flip
  `embedding_multimodal: true` (DB plane via `cortex config set` or env via
...
cortex doctor                # schema_version should be 36+
```

If `cortex doctor` reports a `schema_version` warning, run apply-migrations
manually and check `~/.cortex/upgrade-errors.jsonl` for details.
...
cortex config set embedding_model voyage:voyage-3-large
cortex config set embedding_dimensions 1024

# Required: flip the multimodal gate.
cortex config set embedding_multimodal true

# Optional: enable OCR via gpt-4o-mini (~$0.0003 per image).
cortex config set embedding_image_ocr true
```

Drop image files (PNG, JPG, JPEG, GIF, WEBP, HEIC, AVIF) into your Cortex workspace
repo and run `cortex sync`. The image lands as a `type: image` page with:
...
Then re-run `cortex apply-migrations --yes`. If your provider doesn't ship
...
- `cortex query --image <path>` flag for image-similarity search.
