...
feature_pitch:
  ...
  description: |
    `cortex doctor` on a 52K-page brain used to sit silent for 10+
...
There is no mechanical step. If `cortex upgrade` completed, progress
...
These work on any `cortex` subcommand:
...
```
cortex --progress-json doctor --json
cortex doctor --json --progress-json
```
...
that just pipe `cortex ... | less` should get readable logs, not a
...
`cortex jobs work` (the Minion worker daemon) writes progress to the
DB via `job.updateProgress`, not to stderr. Read per-job progress via
the `get_job_progress` MCP op or:

```bash
cortex jobs submit embed
# while it runs:
cortex jobs get <id>   # .progress updates live as the handler ticks
```
...
`integrity auto` still writes `~/.cortex/integrity-progress.jsonl`,
