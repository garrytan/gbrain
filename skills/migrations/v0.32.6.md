...
feature_pitch:
  ...
  one_liner: "cortex eval suspected-contradictions samples retrieval pairs and flags conflicts, with doctor + MCP + dream-cycle integration."
---

# v0.32.6 Migration: Contradiction Probe

The agent ran this version migration when the user upgraded cortex. After
upgrade, this file is read by the auto-update agent which performs the
following steps automatically.

## Mechanical (handled by `cortex apply-migrations`)
...
- `eval_contradictions_runs` — one row per `cortex eval
...
cortex eval suspected-contradictions --query "what is acme's MRR" --top-k 5 --json

# Run against a queries file:
cortex eval suspected-contradictions --queries-file ~/.cortex/queries.jsonl --top-k 5

# Inspect findings without re-running:
cortex eval suspected-contradictions review --severity high

# Trend over time:
cortex eval suspected-contradictions trend --days 30
```

## Doctor integration

After the first probe run, `cortex doctor` shows a `contradictions` check
...
```bash
cortex doctor --json | jq '.checks[] | select(.name == "contradictions")'
```
...
didn't apply — run `cortex apply-migrations --yes` and re-check.
