...
the skillpack). Then in your `cortex query` calls:
...
4. Omit both for canonical / definitional / code / graph queries
   (`who is X`, `what is X`, etc.) — cortex's heuristic defaults
   to `off`.
...
```bash
# 1. Confirm upgrade
cortex --version    # 0.29.1
cortex doctor --json | jq '.checks[] | select(.name | startswith("schema_version"))'

# 2. Recompute emotional weights (one-time after upgrade)
cortex dream --phase recompute_emotional_weight

# 3. Verify health checks
cortex doctor --json | jq '.checks[] | select(.name | startswith("salience_health") or startswith("effective_date_health"))'

# 4. Try the new axes
cortex query "what's been going on with X" --explain --json | jq '._resolved'
# expected: { salience: "on", recency: "on", salience_source: "auto_heuristic", recency_source: "auto_heuristic" }

cortex query "who is X" --explain --json | jq '._resolved'
# expected: { salience: "off", recency: "off", ... }

# 5. Date filter
cortex query "acme" --since 7d --until 2024-06-30 --json
```
...
cortex reindex-frontmatter --yes --force

# Doctor for any warnings
cortex doctor --json
```

If issues persist, file at https://github.com/Versatly/CortexBrain/issues
with the doctor output and contents of `~/.cortex/upgrade-errors.jsonl`
(if it exists).
