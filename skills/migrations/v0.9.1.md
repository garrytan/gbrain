...
**If your Cortex workspace uses frontmatter `slug:` overrides intentionally**, those pages
...
Run `cortex sync` and check for "Skipped ... does not match path-derived slug"
...
`cortex embed --all` now runs 30x faster (sliding worker pool, 20 concurrent
...
for most brains. Set `CORTEX_EMBED_CONCURRENCY=10` if you hit API rate limits.
...
echo '{"version":"0.9.1","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","status":"complete"}' >> ~/.cortex/migrations/completed.jsonl
```
