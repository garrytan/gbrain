...
users** — re-run `cortex init --migrate-only` and re-run `cortex extract all` if
...
1. **`cortex extract` no longer hangs on large brains.** The N+1 dedup pre-load
...
the v0.12.0 upgrade attempt. Run `cortex init --migrate-only` once to confirm
your schema is at version 10, then run `cortex extract all --dir <brain>` if
...
... + table swap` workaround Cortex posted, your schema should already be at
version 10. Confirm with:

```bash
cortex config get version
```
...
Re-running `cortex init --migrate-only` is a no-op for v9 (uses
...
### Re-running `cortex extract` on a previously-stuck brain

This is the most common case. Run:

```bash
cortex extract all --dir <brain-dir>
# or for live brains with no local checkout:
cortex extract all --source db
```
...
If you hit any issue, file at https://github.com/Versatly/CortexBrain/issues with the
output of `cortex init --migrate-only` and `cortex config get version`.
