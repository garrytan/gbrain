...
version: 0.11.0
feature_pitch:
  headline: "Cortex Minions — durable background agents"
...
**Audience: host agents (Cortex deployments, future
hosts) reading this AFTER `cortex apply-migrations` has run its
...
## Check what cortex left you

```bash
cat ~/.cortex/migrations/pending-host-work.jsonl
```
...
Gbrain rewrites cron entries whose handler name matches a cortex
builtin (`sync`, `embed`, `lint`, `import`, `extract`, `backlinks`,
`autopilot-cycle`). For host-specific handlers (e.g. `ea-inbox-sweep`,
`frameio-scan`, `x-dm-triage`, `calendar-sync` on your Cortex), cortex
...
Autopilot already spawns `cortex jobs work` as a child. Configure it to
spawn your custom worker binary (e.g. `your-cortex-worker`) instead, or
...
One test that exercises the full pipeline: scheduler → `cortex jobs
...
Run `cortex check-resolvable`. It validates reachability (is the skill
...
cron entry from `agentTurn ea-inbox-sweep` to `cortex jobs submit ...`
...
`cortex apply-migrations`, and let cortex re-detect + update
...
- `skills/conventions/cron-via-minions.md` — the rewrite pattern
  cortex applies for builtins + the one your host ships for custom
  handlers.
