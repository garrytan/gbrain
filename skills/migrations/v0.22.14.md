...
feature_pitch:
  ...
  body: |
    Bare `cortex jobs work` now ships with the same health protection the
    ...
    If you're using `cortex jobs supervisor`, you're already protected — the
    supervisor handles spawn-on-crash itself. The fail-stop concern only
    applies to direct `cortex jobs work` invocations.
---
...
If you run `cortex jobs work` directly (NOT under `cortex jobs supervisor`),
...
| cron watchdog | Cron entry that re-spawns when `pgrep -f "cortex jobs work"` is empty |
...
restart policy OR switch to `cortex jobs supervisor` (which spawns its own
child + restarts on crash internally).

## What ships

- DB liveness probes inside `cortex jobs work` (60s interval, 3 strikes → exit)
...
- `CORTEX_SUPERVISED=1` env var (set by supervisor on its child) causes the
...
**`cortex jobs work` now defaults `--max-rss 2048` (MB).** Pre-v0.22.14, bare
...
- You run `cortex jobs work` directly under a process manager (systemd, Docker,
...
- You run `cortex jobs supervisor` (the supervisor already defaulted to
...
If the user runs Cortex, Hermes, or any agent fork that uses `cortex jobs
work` directly (NOT under `cortex jobs supervisor`), they need to know. Check:

```bash
# Look for bare worker invocations in cron, systemd, Docker, launchctl, etc.
grep -r "cortex jobs work" ~/.config/systemd/ /etc/systemd/ 2>/dev/null | head
crontab -l 2>/dev/null | grep "cortex jobs work" | head
launchctl list 2>/dev/null | grep cortex | head
ps -ef | grep "cortex jobs work" | grep -v grep | grep -v supervisor
```
...
> "v0.22.14 changes `cortex jobs work` to default `--max-rss 2048` (MB).
...
**systemd (~/.config/systemd/user/cortex-worker.service):**

```ini
ExecStart=/usr/local/bin/cortex jobs work --queue default --concurrency 3 --max-rss 4096
# Or to opt out: --max-rss 0
```

Then `systemctl --user daemon-reload && systemctl --user restart cortex-worker`.

**cron (`crontab -e`):**

```cron
@reboot /usr/local/bin/cortex jobs work --queue default --concurrency 3 --max-rss 4096
```

**Docker compose:**

```yaml
command: ["cortex", "jobs", "work", "--queue", "default", "--concurrency", "3", "--max-rss", "4096"]
```

**launchctl (~/Library/LaunchAgents/com.user.cortex-worker.plist):**
...
```bash
cortex jobs work --health-interval 0 --max-rss 0
```

### 6. Verify

```bash
cortex jobs stats              # queue should be flowing normally
cortex doctor --json | jq '.'  # no critical warnings
ps -o rss= -p $(pgrep -f "cortex jobs work") | awk '{print $1/1024 " MB"}'
```
...
60s` segment (because `CORTEX_SUPERVISED=1` skips the child's self-monitor).

### 7. If anything fails

Open an issue at https://github.com/Versatly/CortexBrain/issues with:
