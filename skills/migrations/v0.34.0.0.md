...
2. `cortex post-upgrade` → `cortex apply-migrations` → migration v56
...
```bash
cortex edges-backfill --all-sources
```
...
```bash
cortex doctor --json | jq '.checks.code_intel_coverage'
cortex call code_blast --symbol performSync
cortex call code_flow --entry_point runCycle
```
...
- **`cortex wiki`** — depends on clusters.
- **`cortex blast` / `cortex flow` CLI thin-shims.** Use `cortex call
