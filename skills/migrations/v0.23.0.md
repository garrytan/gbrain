...
version: 0.23.0
feature_pitch:
  headline: "cortex dream now actually dreams: conversation transcripts → reflections, originals, and 25-year patterns."
  description: |
    ...
    Synthesize reads conversation transcripts (e.g., Cortex session corpus,
...
**Audience: host agents reading this after `cortex apply-migrations` has
...
`cortex upgrade` chains to `cortex apply-migrations --yes`, which runs:
...
`cortex dream` (and `cortex autopilot`) now run an 8-phase cycle:
...
## To enable synthesize on your Cortex workspace

Three steps. Take them when ready — there is no rush.

```bash
# 1. Point at the directory where your conversation transcripts live.
#    Cortex stores session transcripts at memory/.dreams/session-corpus/<YYYY-MM-DD>.txt
...
cortex dream --phase synthesize --dry-run --json
```
...
cortex config set dream.synthesize.min_chars 2000
...
cortex config set dream.synthesize.exclude_patterns '["medical","therapy"]'
...
cortex config set dream.synthesize.cooldown_hours 12

# Patterns lookback window in days (default 30).
cortex config set dream.patterns.lookback_days 30

# Minimum distinct reflections needed to name a pattern (default 3).
cortex config set dream.patterns.min_evidence 3
```
...
- **Auto git commit + push.** v1 writes markdown files to `brain_dir`
  ...
  or let `cortex autopilot` handle it. v1.1 will add explicit
...
cortex dream --help    # shows the 8-phase pipeline

# Dry-run against a single transcript (cheap Haiku call only):
cortex dream --phase synthesize --input /tmp/some-transcript.txt --dry-run --json
```

If any step fails, file an issue with `cortex doctor` output and the
