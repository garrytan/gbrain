...
version: 0.17.0
feature_pitch:
  headline: "One brain maintenance cycle, two CLIs. `cortex dream` delivers the README promise."
  description: |
...
# v0.17.0 Migration: `cortex dream` + unified maintenance cycle
...
cortex dream --dry-run --json    # preview, agent-readable
cortex dream --phase lint        # single-phase (fast, targeted)
cortex dream --pull              # git pull before syncing
0 2 * * * cortex dream --json    # nightly cron
```

See `cortex dream --help` for the full flag reference.

### Autopilot now runs lint + orphan sweep

`cortex autopilot --install` users: on upgrade, your daemon's cycle
...
- **orphan sweep** — reports (read-only) pages with no inbound
  wikilinks. Visible in `cortex jobs list` output for each
  `autopilot-cycle` job.
...
`cortex_cycle_locks` (new table, migration v16) replaces
...
cortex jobs list --status complete | head -5
# Each `autopilot-cycle` entry now has 6 phases in its report,
# not 4. Check a recent one with `cortex jobs get <id>`.

# 4. Schema migration landed:
cortex doctor  # should show no pending migrations
```

Expected `cortex dream --dry-run` output on a healthy brain:
...
0 2 * * * cortex dream --phase orphans
```
...
go wrong, `cortex dream --dry-run` always tells you what WOULD
...
**`cortex dream --dry-run` reports changes when you expected none:**
Check `cortex doctor` for drift: lint issues, stale embeddings,
...
Open a GitHub issue with the output of `cortex jobs get <id>` for a
...
- `cortex autopilot --install` machinery (launchd / systemd /
  crontab generators). Existing installs keep working.
- `~/.cortex/autopilot.lock` daemon-singleton lockfile. Separate
  concern from the new per-cycle lock.
- `cortex jobs` interface. `cortex jobs get <id>` now shows a
...
required — all changes apply automatically on `cortex upgrade`.*
