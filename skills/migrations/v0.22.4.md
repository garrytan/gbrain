...
- `cortex frontmatter` CLI (validate / audit / install-hook)
- `frontmatter_integrity` subcheck under `cortex doctor`
...
- Audit-only migration that scans every registered source, writes
  `~/.cortex/migrations/v0.22.4-audit.json`, and queues per-source TODO
  entries to `~/.cortex/migrations/pending-host-work.jsonl`
- 0 warnings on `cortex check-resolvable` (down from 7 on master)
...
- `audit` — `scanBrainSources()` over every registered source; writes
  `~/.cortex/migrations/v0.22.4-audit.json`
- `emit-todo` — appends one entry per source-with-issues to
  `~/.cortex/migrations/pending-host-work.jsonl`. Each entry contains the
  exact `cortex frontmatter validate <source-path> --fix` command.
...
```bash
cat ~/.cortex/migrations/v0.22.4-audit.json
```
...
```bash
cortex frontmatter validate <source_path> --fix
```

`--fix` writes a `.bak` backup for every modified file. SLUG_MISMATCH errors
are surfaced for manual review (not auto-fixed) — cortex derives slugs from
...
```bash
cortex doctor --json | jq '.checks[] | select(.name == "frontmatter_integrity")'
cortex frontmatter audit --json | jq '.total'
```
...
- contents of `~/.cortex/migrations/v0.22.4-audit.json`
