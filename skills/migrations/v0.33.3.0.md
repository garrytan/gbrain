...
`cortex upgrade` runs `cortex apply-migrations` automatically. Most users
...
- **CLI source-scoping default flipped** (Codex finding #7): `cortex
  code-callers <symbol>` without `--source` now resolves to your Cortex workspace's
...
unless you watch `cortex doctor`'s autopilot stats.

## What the agent should do

If you're an agent (Claude Code, Cortex, Cursor) running through MCP:
...
The previously available CLI commands (`cortex code-callers <symbol>`)
...
```bash
# 1. Confirm upgrade
cortex --version    # 0.33.3.0

# 2. Confirm migration v51 ran
cortex apply-migrations --status | grep edges_backfilled_at_v0_34

# 3. Confirm the new MCP ops are exposed
cortex --tools-json | jq '.[] | select(.name | startswith("code_")) | .name'
...
cortex code-callers parseMarkdown --json | jq '.count'

# 5. (Optional) inspect the resolver's per-source progress
cortex doctor --json | jq '.checks | to_entries[] | select(.key | contains("code"))'
```

## Rollback

If you need to roll back: `git checkout` an older binary and run
`cortex doctor`. The v51 column + indexes stay in your DB (no-op for
older binaries; idempotent re-add on upgrade). No data is destroyed.

## What v0.34 will add on top

Per the v0.34 design doc (`/Users/cortex/.cortex/projects/cortex-cortex/cortex-cortex-miami-design-20260510-194145.md`):
...
- `cortex wiki` zero-LLM aggregator CLI
