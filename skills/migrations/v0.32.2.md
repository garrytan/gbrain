...
feature_pitch:
  ...
  description: "v0.31 added hot-memory facts but they lived only in the database. v0.32.2 fences them in markdown so they survive `cortex rebuild`. Migration v0_32_2 backfills existing facts to entity-page fences. Plus a 3-layer privacy strip and a CI gate that enforces the rule going forward."
  recipe: null
  tiers: null
---
...
1. Run `cortex apply-migrations --yes` to invoke the v0_32_2 orchestrator.
2. The orchestrator runs in two phases:
   ...
   - **phaseBVerify** re-parses every touched fence + diffs against
     ...
     `cortex apply-migrations --force-retry 51`.
...
1. **`cortex forget <id>` now rewrites markdown.** Previously `cortex
   forget` called `engine.expireFact(id)` which UPDATEd the DB only.
   After `cortex rebuild` (v0.32.3) the forget would evaporate. v0.32.2
...
4. **CI invariant gate.** `scripts/check-system-of-record.sh` is wired
   ...
   sites carry `// cortex-allow-direct-insert: <reason>` comments on
...
test ! -f ~/.cortex/facts.write_failures.jsonl || echo "WARN: some failures recorded"

# Each entity page with v0.31 facts now has a fence
ls people/ companies/ | head -5
grep -l 'cortex:facts:begin' people/*.md | head -5
...
- **Don't call `engine.insertFact` / `expireFact` from new code paths.**
  ...
  for a new reconciler), add the `// cortex-allow-direct-insert:
