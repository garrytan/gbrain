...
# v0.36.5.0 — Free-form secret inheritance for shell jobs calling `cortex` CLI
...
- **Shell-job pre-enqueue validator** (`src/core/minions/handlers/shell-validate.ts`,
  ...
  `cortex jobs submit shell` (CLI) and `submit_job` MCP op for `name='shell'`.
...
- **Env-key derivation**: name uppercased by default. One override:
  `database_url` → `CORTEX_DATABASE_URL` (plain `DATABASE_URL` is ambiguous
...
- **Defense-in-depth re-validation** in the shell handler at job-pickup
  ...
  pre-enqueue call.
- **`cortex doctor home_dir_in_worktree` check** warns when `~/.cortex/`
  lives inside a git worktree. Handles `.git` as directory or file.
- **`~/.cortex/.gitignore` retroactive** via `ensureGitignore()` called from
  `saveConfig()` AND `cortex post-upgrade`. Idempotent, never clobbers.
- **Output-side redaction (opt-in)**: set `redact_secrets: true` on the job
  ...
  `echo "$CORTEX_DATABASE_URL"`, not adversarial encode-then-print. Default
  false (back-compat).
- New canonical guide: `docs/guides/agent-to-cortex.md`. Two-domain framing
...
under the derived key (`database_url` → `CORTEX_DATABASE_URL`,
...
with a paste-ready `cortex config set <name> <value>` hint.
...
cortex jobs submit shell --params \
  '{"cmd":"cortex stats","cwd":"/tmp","inherit":["database_url"]}' --follow
# Expect: page count, exit 0.

# 2. The doctor surfaces worktree risk if it exists:
cortex doctor --json | grep -A1 home_dir_in_worktree

# 3. The retroactive gitignore landed:
test -f ~/.cortex/.gitignore && cat ~/.cortex/.gitignore
# Expect: file exists, contents = "*\n"

# 4. The audit-log records names not values:
tail -1 ~/.cortex/audit/shell-jobs-*.jsonl | grep -o '"inherit":\[[^]]*\]'
...
jobs: write `database_url` plaintext to `~/.cortex/config.json`, or pass
`env: { CORTEX_DATABASE_URL: ... }` per-job. Both work. Both leave plaintext
