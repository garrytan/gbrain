...
Universal migration from any wiki, note tool, or brain system into Cortex.

## Contract

- Source data is never modified or deleted; migration is additive only.
- Every migrated page is verified round-trip: written to cortex, read back, spot-checked.
- Cross-references from the source system (wikilinks, block refs, tags) are converted to cortex equivalents.
...
|--------|--------|----------|
| Obsidian | Markdown + `[[wikilinks]]` | Direct import, convert wikilinks to cortex links |
...
| Plain markdown | Any .md directory | Import directory into cortex directly |
...
2. **Plan the mapping.** How do source fields map to cortex fields (type, title, tags, compiled_truth, timeline)?
3. **Test with a sample.** Import 5-10 files, verify by reading them back from cortex and exporting.
4. **Bulk import.** Import the full directory into cortex.
5. **Verify.** Check cortex health and statistics, spot-check pages.
6. **Build links.** Extract cross-references from content and create typed links in cortex.

## Obsidian Migration

1. Import the vault directory into cortex (Obsidian vaults are markdown directories)
2. Wire the graph with native wikilink support (v0.12.1+):

   ```bash
   cortex extract links --source db --dry-run | head -20    # preview
   cortex extract links --source db                         # commit
   ```
...
Obsidian-specific:
- Tags (`#tag`) become cortex tags
- Frontmatter properties map to cortex frontmatter
...
5. Import the cleaned directory into cortex
...
4. Store each page in cortex

## Verification

After any migration:
1. Check cortex statistics to verify page count matches source
2. Check cortex health for orphans and missing embeddings
3. Export pages from cortex for round-trip verification
4. Spot-check 5-10 pages by reading them from cortex
5. Test search: search cortex for "someone you know is in the data"
...
- **Ignoring cross-references.** Wikilinks, block refs, and tags from the source system must be converted to cortex equivalents. Dropping them loses the knowledge graph.
...
```
MIGRATION REPORT -- [source] -> Cortex
=======================================
...
- Store/update pages in cortex (put_page)
- Read pages from cortex (get_page)
- Link entities in cortex (add_link)
- Tag pages in cortex (add_tag)
- Get cortex statistics (get_stats)
