...
- Implements the upstream GitHub awesome-copilot skill intent for `arize-trace`: Downloads, exports, and inspects existing Arize traces and spans to understand what an LLM app is doing or debug runtime issues. Covers exporting traces by ID, spans by ID, sessions by ID, and root-cause investigation us
