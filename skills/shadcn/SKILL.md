...
- Manages shadcn/ui components and projects: adding, searching, fixing, debugging, styling, and composing UI, including chat interfaces and component registries.
- Applies when working with shadcn/ui, component registries, presets, `--preset` codes, or any project with a `components.json`.
- Prefers the shadcn CLI (`npx shadcn@latest`, `pnpm dlx shadcn@latest`, `bunx --bun shadcn@latest`) for add/init operations rather than hand-editing component files.
- Loads the relevant `rules/*.md` and `reference/*.md` only when the task needs them, to keep context lean.

## Output Format

Replies reference concrete component paths, registry URLs, and CLI commands verbatim. When demonstrating composition, show the minimal code and the `cn()`/variant wiring; do not dump full component source unless asked. Surface the exact CLI invocation used.

## Anti-Patterns

- Hand-editing generated component files instead of using the shadcn CLI.
- Inventing component names not in the registry.
- Applying a preset without checking `components.json` compatibility.
- Dumping entire component source into context when a doc link or the variant signature suffices.
