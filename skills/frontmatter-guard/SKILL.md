...
description: |
  ...
  `cortex frontmatter` CLI for agent-driven workflows.
triggers:
...
- Validation logic is shared with `cortex doctor`'s `frontmatter_integrity` subcheck — single source of truth
- Reports per source (cortex is multi-source since v0.18.0); never silently audits the wrong root
...
Without a guard, these accumulate silently until `cortex sync` chokes or search returns garbage. The guard makes the failure visible at audit time and trivially fixable.
...
```bash
cortex frontmatter validate <path> --json
```
...
```bash
cortex frontmatter validate <path> --fix
```
...
```bash
cortex frontmatter install-hook [--source <id>]
```

The hook runs `cortex frontmatter validate` against staged `.md`/`.mdx` files. Bypass with `git commit --no-verify`.
...
- Always run `cortex frontmatter audit --json` first; never assume a brain is clean.
...
- `SLUG_MISMATCH` fixes remove the frontmatter `slug:` field — cortex derives slug from path. Mention this when the user's title is intentionally renamed.
...
- `cortex doctor` — the `frontmatter_integrity` subcheck reports the same counts as `audit`.
- `skills/maintain/SKILL.md` — broader brain health audit; chain after this skill if other classes of issue are suspected.
- `skills/lint/SKILL.md` (via `cortex lint`) — overlapping rules for skill-file lint; the `frontmatter-*` rule names in lint output come from this skill's validation surface.
...
Fix with: cortex frontmatter validate /Users/me/brain --fix
```
...
`cortex frontmatter validate <path> --json` returns a similar envelope keyed on per-file results instead of per-source.
...
```yaml
# Correct: single-quoted YAML flow (canonical form cortex emits)
tags: ['yc', 'w2025', 'ai']
...
**Don't use `--fix` to "make doctor green" without reading the audit first.** SLUG_MISMATCH cases are surfaced for manual review specifically because cortex derives the slug from path. A mismatch usually means the user renamed a file intentionally; auto-removing the slug field is the right outcome only when you've confirmed the rename was deliberate.
...
**Don't run `audit` on a brain where sources aren't registered.** The CLI returns "no registered sources to audit" gracefully, but the migration emits a `skipped: no_sources` phase result. Don't paper over this with a manual path-walk; the right fix is to register the source via `cortex sources add`.
