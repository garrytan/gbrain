...
triggers:
  - "aws-resource-query"
  - "aws resource query"
  - "awesome copilot aws resource query"
---
...
- Implements the upstream GitHub awesome-copilot skill intent for `aws-resource-query`: Query AWS resources using natural language. Covers EC2, S3, RDS, Lambda, ECS, EKS, Secrets Manager, IAM, VPC, networking, messaging, and more. Strictly read-only — no writes, deletes, or mutations.
