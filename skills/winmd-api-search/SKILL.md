...
- Implements the upstream GitHub awesome-copilot skill intent for `winmd-api-search`: Find and explore Windows desktop APIs. Use when building features that need platform capabilities — camera, file access, notifications, UI controls, AI/ML, sensors, networking, etc. Discovers the right API for a task and
