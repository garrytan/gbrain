...
- Implements the upstream GitHub awesome-copilot skill intent for `ruff-recursive-fix`: Run Ruff checks with optional scope and rule overrides, apply safe and unsafe autofixes iteratively, review each change, and resolve remaining findings with targeted edits or user decisions.
