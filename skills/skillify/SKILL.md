...
> skill's Phase 3 below uses `cortex eval cross-modal` instead — three
...
receipt under `~/.cortex/.cortex/eval-receipts/<slug>-<sha8>.json` (the
...
Models resolve through the cortex AI gateway. Configure once with:
...
from `~/.cortex/config.json` plus `process.env`.
...
cortex skillify check skills/<slug>/scripts/<slug>.mjs --json | \
  jq '.[] | .items[] | select(.name | contains("Cross-modal"))'
ls ~/.cortex/.cortex/eval-receipts/              # receipt landed
cortex check-resolvable --json | jq .ok          # resolver clean
```
...
1. **The skill tree on disk.** `skills/<slug>/SKILL.md`, `scripts/<slug>.mjs`,
   ...
   `cortex skillify scaffold <name>` and refined by the human/agent into a
   real implementation.
2. **A cross-modal eval receipt** at
   `~/.cortex/.cortex/eval-receipts/<slug>-<sha8>.json`. The sha-8 binds the
   receipt to the current `SKILL.md` content. `cortex skillify check`
   surfaces the status (`found` / `stale` / `missing`) as informational.
3. **An audit verdict** from `cortex skillify check`: `properly skilled` |
...
JSON output (`cortex skillify check --json`) includes the same fields plus
