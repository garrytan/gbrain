...
version: 0.33.0
description: |
  Lift a proven skill from a host repo back into Cortex's bundle so other
  clients can scaffold it. Editorial workflow:
...
# skillpack-harvest — Editorial workflow for lifting host skills into cortex

> **Convention:** see [_brain-filing-rules.md](../_brain-filing-rules.md) for
> file placement rules. This skill writes into cortex's own tree, not the
> brain repo's notes.

This skill is the inverse of `cortex skillpack scaffold`. Scaffold ships
skills downstream (cortex → host). Harvest lifts proven patterns
upstream (host → cortex) so they become references every other client
can scaffold.
...
3. `cortex skillpack harvest --dry-run` previewed the file set.
4. The real `cortex skillpack harvest <slug> --from <host>` succeeded
   with `status: harvested` (no privacy-lint hits).
5. `bun test test/skills-conformance.test.ts` passes on the new
   `skills/<slug>/SKILL.md`.
6. The user has reviewed the diff in cortex and explicitly approved
   the commit.

If any of these is incomplete, the skill is NOT yet harvested — the
files may sit in cortex's working tree, but they're not landed.

## Output Format

This skill produces three artifacts in cortex's working tree:
...
3. An updated `cortex.plugin.json` with the new slug added to
   `skills:` (sorted)
...
- **Skipping the dry-run.** Always preview first. Files land in
  cortex's working tree; cleanup is a `git checkout` away, but you
  shouldn't need to.
...
- **Harvesting a skill that's still in flux.** Wait until the host
  ...
  then re-harvest, and that churns cortex's bundle for no benefit.
...
- The user developed a skill in their host fork (Wintermute, Neuromancer,
  Zion, etc.) and wants other cortex clients to be able to use it
...
3. **The user owns the cortex checkout.** The harvest writes into
   cortex's working tree. They'll review and commit. Don't harvest
...
- What slug should the harvested skill have? (Slugs must be kebab-case,
  globally unique in the cortex bundle.)
...
```bash
cortex skillpack harvest <slug> --from <host-repo-root> --dry-run
```

The output shows:
- Which files would land in cortex's tree
...
1. **Fork-specific names → generic phrasing**
   - `Wintermute` → `your Cortex` (or `Cortex deployment`)
...
3. **Fork-specific conventions → references**
   - Mentions of `<host-repo>/docs/...` files → either lift the doc
     into cortex OR replace with a generic placeholder explanation
...
2. `cortex skillpack check --strict` — confirms no drift between
   bundle and cortex's own checkout.
3. `cortex skillpack list` — confirms the slug shows up in the bundle.
4. Review the diff: `cd <cortexRoot> && git diff -- skills/<slug>/`
5. Commit the additions in cortex (do NOT commit any leftover files
   in the host repo — harvest is a copy, not a move).

### Phase 6 — Downstream announcement (optional)

If other cortex clients should pick up the new skill:
...
```bash
cortex skillpack harvest <slug> --from <host-repo-root> --no-lint
```
...
- It does NOT publish to npm or a remote bundle. It writes to
  cortex's working tree; the user commits + ships via the normal
...
- cortex's `skills/<slug>/` — every file in the host skill dir
  (copy)
- cortex's mirror path for declared paired sources
  (e.g. `src/commands/<slug>.ts` if the host SKILL.md declares it
  in frontmatter)
- cortex's `cortex.plugin.json` — adds the slug to `skills:`
