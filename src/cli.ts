...
const CLI_ONLY = new Set(['init', 'reinit-pglite', 'upgrade', 'post-upgrade', 'check-update', 'integrations', 'publish', 'check-backlinks', 'lint', 'report', 'import', 'import-iphone', 'export', 'files', 'embed', 'serve', 'call', 'config', 'doctor', 'migrate', 'eval', 'sync', 'extract', 'extract-conversation-facts', 'features', 'autopilot', 'graph-query', 'jobs', 'agent', 'apply-migrations', 'skillpack-check', 'skillpack', 'resolvers', 'integrity', 'repair-jsonb', 'orphans', 'sources', 'mounts', 'dream', 'check-resolvable', 'routing-eval', 'skillify', 'smoke-test', 'providers', 'storage', 'repos', 'code-def', 'code-refs', 'reindex', 'reindex-code', 'reindex-frontmatter', 'code-callers', 'code-callees', 'frontmatter', 'auth', 'friction', 'claw-test', 'book-mirror', 'takes', 'think', 'salience', 'anomalies', 'transcripts', 'models', 'remote', 'recall', 'forget', 'edges-backfill', 'cache', 'ze-switch', 'founder', 'brainstorm', 'lsd', 'schema', 'capture', 'onboard', 'conversation-parser', 'status', 'skillopt']);
...
const THIN_CLIENT_REFUSED_COMMANDS = new Set([
  'sync', 'embed', 'extract', 'extract-conversation-facts', 'import-iphone', 'migrate', 'apply-migrations',
...
const THIN_CLIENT_REFUSE_HINTS: Record<string, string> = {
  ...
  'import-iphone': 'import-iphone reads decrypted local iPhone backup files and must run on the host machine with the local brain.',
...
async function handleCliOnly(command: string, args: string[]) {
  ...
  if (command === 'import-iphone' && (args.includes('--help') || args.includes('-h'))) {
    const { runImportIphone } = await import('./commands/import-iphone.ts');
    await runImportIphone(null, args);
    return;
  }
  ...
  try {
    switch (command) {
      ...
      case 'import-iphone': {
        const { runImportIphone } = await import('./commands/import-iphone.ts');
        await runImportIphone(engine, args);
        break;
      }
      case 'export': {
