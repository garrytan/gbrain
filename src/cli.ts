...
const CLI_ONLY_SELF_HELP = new Set([
  ...
  'self-upgrade',
  // #2570: `gbrain generated` ships its own subcommand/bucket help
  // (src/commands/generated.ts HELP constant).
  'generated',
...
const THIN_CLIENT_REFUSED_COMMANDS = new Set([
  ...
  // #2570: generated-pages report runs raw SQL against the host's engine;
  // thin clients use the `list_generated_pages` MCP op instead.
  'generated',
...
const THIN_CLIENT_REFUSE_HINTS: Record<string, string> = {
  ...
  generated: "generated needs the host's brain. Run on the host or use the `list_generated_pages` MCP tool from your agent.",
...
async function handleCliOnly(command: string, args: string[]) {
  ...
  // #2570: same pattern for `gbrain generated` help. `generated` is in
  ...
  // `gbrain generated` usage screen) without a configured brain.
  // runGenerated's help path returns before touching the engine.
  if (
    command === 'generated' &&
    (args.length === 0 || args.includes('--help') || args.includes('-h'))
  ) {
    const { runGenerated } = await import('./commands/generated.ts');
    await runGenerated(null as never, args);
    return;
  }
  ...
  try {
    switch (command) {
      ...
      case 'generated': {
        ...
        const { runGenerated } = await import('./commands/generated.ts');
        await runGenerated(engine, args);
        break;
      }
      case 'maintain': {
...
TOOLS
  ...
  generated list [--json]            Adoption report over dream-generated pages (#2570, read-only)
