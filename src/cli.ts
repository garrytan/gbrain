...
async function handleCliOnly(command: string, args: string[]) {
  ...
  try {
    switch (command) {
      ...
      case 'article-mirror': {
        const { runArticleMirrorCmd } = await import('./commands/article-mirror.ts');
        await runArticleMirrorCmd(engine, args);
        break;
      }
      case 'sync': {
        const { runSync } = await import('./commands/sync.ts');
