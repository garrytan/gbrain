...
/**
 * The only database query performed by `gbrain doctor --fast`.
 * Connection setup suppresses its eager Postgres SELECT so this seam owns the
 * single lightweight round trip on both Postgres and PGLite.
 */
...
export async function runDoctor(
  ...
  const checks = await buildChecks(engine, args, dbSource, connectionError);
