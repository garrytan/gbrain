/**
 * `cortex reinit-pglite` — wipe-and-reinit PGLite brain in one command.
 *
 ...
 *
 *   mv ~/.cortex/brain.pglite ~/.cortex/brain.pglite.bak
 *   cortex init --pglite --embedding-model X --embedding-dimensions N
 *   cortex sync
 *
...
import { loadConfig, loadConfigFileOnly, gbrainPath as cortexPath } from '../core/config.ts';
...
export async function runReinitPglite(args: string[]): Promise<void> {
  ...
  if (cfg?.engine !== 'pglite') {
    fail(
      opts.jsonOutput,
      'not_pglite',
      `cortex reinit-pglite is for PGLite brains only (current engine: ${cfg?.engine || 'none'}). ` +
  ...
  const dbPath = opts.customPath
    || cfg.database_path
    || cortexPath('brain.pglite');

  if (!existsSync(dbPath)) {
    fail(
      opts.jsonOutput,
      'no_brain',
      `No PGLite brain found at ${dbPath}. Run \`cortex init --pglite\` to create one.`,
    );
  }
  ...
  if (!opts.jsonOutput) {
    console.log('');
    console.log('cortex reinit-pglite — wipe and re-create the PGLite brain.');
    console.log('');
  ...
  if (!opts.noSync) {
    ...
    try {
      ...
      if (!newCfg) {
        if (!opts.jsonOutput) console.error('Warning: no config after reinit; skipping sync. Run `cortex sync` manually.');
        return;
      }
      ...
      if (!opts.jsonOutput) {
        ...
        console.error('The brain is initialized but empty. Run \`cortex sync\` to populate it.');
      }
    }
  }
...
function printHelp(): void {
  console.log(`Usage: cortex reinit-pglite [options]
...
Optional:
  --path <path>                        Active brain path (default: ~/.cortex/brain.pglite).
  --yes / -y                           Skip the TTY confirmation prompt.
  --no-sync                            Skip the post-init \`cortex sync\`.
  --json                               Emit structured JSON output on stdout.

Examples:
  # Switch from OpenAI/1536 to ZeroEntropy/1280:
  cortex reinit-pglite --embedding-model zeroentropyai:zembed-1 --embedding-dimensions 1280

  # Skip the sync step (do it later):
  cortex reinit-pglite --embedding-model openai:text-embedding-3-large \\
    --embedding-dimensions 1536 --no-sync

The old brain is preserved as \`<path>.bak\`. To roll back, mv it back.

See also:
  cortex doctor                        Diagnose dim mismatches before/after.
