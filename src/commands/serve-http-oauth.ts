...
/**
 * Add an OAuth scope hint to SDK-generated Bearer challenges without using
 * `requiredScopes`, which would also enforce literal scopes at the transport
 * boundary and reject valid implied-scope grants such as `agent`.
 */
export function withBearerScopeHint(middleware: RequestHandler, scopes: readonly string[]): RequestHandler {
  ...
  return async (req, res, next) => {
    ...
    res.set = function (field: string | Record<string, string | string[]>, value?: string | string[]) {
