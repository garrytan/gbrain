/**
 * cortex bench publish — turn a captured-run NDJSON into a baseline file (v0.41).
 *
 ...
 * of `cortex eval export` and writes a `*.baseline.ndjson` file with:
 ...
 *
 * Usage:
 *   cortex bench publish --from <captured.ndjson> --to <X.baseline.ndjson>
...
function printHelp(): void {
  console.log(`cortex bench publish — write a baseline file from captured queries

Usage:
  cortex bench publish --from <captured.ndjson> --to <X.baseline.ndjson> [flags]

Required:
  --from FILE                    Input NDJSON from \`cortex eval export\`
...
Examples:
  # Publish your own tenant baseline
  cortex eval export --limit 200 --tool query > /tmp/captured.ndjson
  cortex bench publish --from /tmp/captured.ndjson --to ~/.cortex/baselines/tenant.baseline.ndjson --label "tenant-2026-05"

  # Gate against it later
  cortex eval gate --baseline ~/.cortex/baselines/tenant.baseline.ndjson
`);
}
