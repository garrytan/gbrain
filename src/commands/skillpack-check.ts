/**
 * `cortex skillpack-check` — agent-readable health report.
 *
 * Wraps `cortex doctor --json` + `cortex apply-migrations --list` into a
 * single JSON blob a host agent or scheduled runtime can consume without
 * parsing two subcommands.
 *
 * Usage:
 *   cortex skillpack-check              # pretty-printed JSON + exit code
 *   cortex skillpack-check --quiet      # only exits with status; no output
 *   cortex skillpack-check --help
 *
 * Exit codes:
...
/**
 * Resolve the Cortex binary + args for spawning subcommands from
 * within skillpack-check. Handles three install cases:
 *   - Running the compiled binary (argv[1] ends in /cortex): re-exec it.
 *   - Running via `bun run src/cli.ts` (argv[1] is a .ts file): prefix with `bun run`.
 *   - Anything else: fall back to `cortex` on $PATH.
 */
function cortexSpawn(): { cmd: string; prefix: string[] } {
  ...
  if (execPath.endsWith('/cortex') || execPath.endsWith('\\cortex.exe') || execPath.endsWith('/gbrain') || execPath.endsWith('\\gbrain.exe')) {
    return { cmd: execPath, prefix: [] };
  }
  return { cmd: 'cortex', prefix: [] };
}
...
function cortexAction(action: string): string {
  return action
...
function runMigrationsList(): SkillpackReport['migrations'] {
  ...
  try {
    ...
    //   Installed cortex version: 0.11.1
    //
...
function buildReport(): SkillpackReport {
  ...
  if ('checks' in doctor) {
    for (const check of doctor.checks) {
      if (check.status === 'fail') {
        ...
        if (runMatch) actions.push(cortexAction(runMatch[1].trim()));
        else actions.push(cortexAction(`[${check.name}] ${check.message}`));
      } else if (check.status === 'warn') {
        // Warnings don't fail the report but surface as informational
        // actions the agent can decide about.
        const runMatch = check.message.match(/Run:\s*(.+)$/);
        if (runMatch) {
          const action = cortexAction(runMatch[1].trim());
    ...
    healthy = false;
    actions.push(cortexAction('Investigate doctor failure: ' + doctor.error));
  }
  ...
  if ('applied_count' in migrations) {
    if (migrations.partial_count > 0 || migrations.pending_count > 0) {
      healthy = false;
      const action = 'cortex apply-migrations --yes';
      if (!actions.includes(action)) actions.unshift(action);
    }
  } else {
    healthy = false;
    actions.push(cortexAction('Investigate apply-migrations failure: ' + migrations.error));
  }

  const summary = healthy
    ? 'cortex skillpack healthy'
    : `cortex skillpack needs attention: ${actions.length} action(s) — ${actions[0]}`;
...
export async function runSkillpackCheck(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`cortex skillpack-check — agent-readable health report.

Wraps doctor + apply-migrations --list into one JSON blob.

Usage:
  cortex skillpack-check            Pretty JSON to stdout, exit 0/1/2 (legacy).
  cortex skillpack check            v0.33 subcommand. Default: informational
                                     (exit 0 even with drift). Pass --strict
                                     to exit non-zero on action-needed.
  cortex skillpack-check --quiet    Exit code only, no output.
...
`);
  ...
  // v0.33: When invoked as the new `cortex skillpack check` subcommand,
  ...
  //
  // Top-level `cortex skillpack-check` (cron compat) keeps exit-1 on
...
/**
 * Detect whether this invocation came via `cortex skillpack check`
 * (subcommand) vs the top-level `cortex skillpack-check` (cron compat).
 * Subcommand → informational default. Top-level → strict default.
 */
function isSkillpackCheckSubcommand(): boolean {
  const argv = process.argv;
  // argv shape for `cortex skillpack check` after the binary name and
...
export const __testing = { buildReport, runDoctor, runMigrationsList, cortexSpawn };
