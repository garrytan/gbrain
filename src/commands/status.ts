/**
 * `cortex status` — single-screen company brain health dashboard.
 *
 * The command that answers "is my brain healthy and working?" without
 * making operators run five other commands (cortex sources status, cortex
 * stats, cortex jobs supervisor status, cortex jobs list, tail audit logs).
 *
 * Six sections:
 ...
 *   - Locks      — active rows in the cycle lock table
...
import { gbrainPath as cortexHomePath, loadConfig, isThinClient } from '../core/config.ts';
...
async function buildQueueCounts(engine: BrainEngine): Promise<QueueCounts> {
  ...
  try {
    ...
    // jobs are the failure mode `cortex status` should surface, not hide.
    const rows = await engine.executeRaw<Row>(
  ...
  if (report.autopilot) {
    ...
    if ('local_only_remote' in report.autopilot) {
      ...
      if (a.running) {
        ...
        lines.push(`  stale lockfile (PID ${a.pid ?? '?'} not alive). Run \`cortex autopilot --install\` to restart.`);
      } else {
        lines.push('  not running. Install with `cortex autopilot --install`.');
      }
    }
    lines.push('');
  }
...
export async function runStatus(
  ...
  if (sectionFlag === 'usage_error') {
    stderr(
      `cortex status: invalid --section. Valid: ${VALID_SECTIONS.join('|')}\n`,
    );
    return { exitCode: 2 };
  }
  ...
  try {
    if (useThinClient) {
      ...
      if (!engine) {
        stderr('cortex status: no engine connected (DB unreachable?). Run `cortex doctor` to diagnose.\n');
        return { exitCode: 1 };
      }
      report = await buildLocalReport(engine, { sections });
    }
  } catch (err) {
    stderr(`cortex status: snapshot failed: ${(err as Error).message}\n`);
    return { exitCode: 1 };
  }
