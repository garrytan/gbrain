...
function writeWrapperScript(repoPath: string): string {
  ...
  // Wrapper sources stable env locations and also exports a minimal PATH so
  // /usr/bin/env bun works under non-interactive launches like systemd/WSL.
...
# Sources stable env files for API keys, then runs autopilot
set -euo pipefail
  ...
  if (process.platform === 'darwin') {
    ...
    if (existsSync(systemdUnitPath())) {
      ...
      try {
        ...
        installed = crontab.includes('gbrain autopilot') || crontab.includes('autopilot-run.sh');
      } catch { /* no crontab */ }
    }
  }
