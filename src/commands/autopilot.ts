      ...
      try {
        ...
        if (engine.kind === 'postgres') {
          try {
            ...
            if (!(await packDeclaresPhase(engine, 'extract_atoms'))) {
              // Policy (enabled flag, max_usd_per_day daily cap, fairness,
              ...
              const dispatched = await dispatchAutoDrains(engine, queue, {
                ...
                onSkip: (sourceId, reason) => process.stderr.write(
                  (jsonMode ? JSON.stringify({ event: 'freshness_source_path_skipped', source_id: sourceId, reason }) : reason) + '\n',
                ),
              });
              for (const d of dispatched) {
                const backlog = d.backlog.pages;
                if (jsonMode) {
                  process.stderr.write(JSON.stringify({
                    event: 'dispatched', job_id: d.jobId, mode: 'auto-drain',
                    source_id: d.sourceId, backlog, transcripts: d.backlog.transcripts,
                  }) + '\n');
                } else {
