...
export async function runAutopilot(engine: BrainEngine, args: string[]) {
  ...
  const engineConfig = cfg ? toEngineConfig(cfg) : undefined;
  ...
  while (!stopping) {
    ...
    try {
      ...
      try {
        ...
        if (engineConfig) {
          await (engine as any).connect?.(engineConfig);
        } else {
    ...
    if (useMinionsDispatch) {
      ...
      try {
        ...
        const report = await runCycle(engine, {
          ...
          // For local vaults without a valid remote, --no-pull keeps autopilot healthy.
          pull: !noPull,
    ...
    if (engineType === 'pglite') {
      try {
        await engine.disconnect();
        console.log('[autopilot] PGLite connection released until next cycle');
      } catch (e) { logError('disconnect-before-sleep', e); }
    }
...
async function installDaemon(engine: BrainEngine, args: string[]) {
  ...
  const noPull = args.includes('--no-pull');
  const wrapperPath = writeWrapperScript(repoPath, { noPull });
  const home = process.env.HOME || '';
