...
export async function cmdReferenceHarness(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'gbrain skillpack reference --harness <h> [<slug>] [--skill <slug>] [--dest PATH] [--scope user|project] [--workspace PATH] [--apply-clean-hunks] [--dry-run] [--json]\n\n' +
        ...
        '--apply-clean-hunks needs exactly one skill (<slug> or a single --skill).',
    );
    process.exit(0);
  }
