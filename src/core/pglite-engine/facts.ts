...
import { staleFactRowToRow } from '../utils.ts';
  ...
  const { rows } = await deps.db.query(
    `SELECT id AS fact_id, fact, entity_slug
      ...
      WHERE embedding IS NULL AND expired_at IS NULL
        ...
        AND NOT (source = ANY($4::text[]))
        AND id > $2
      ORDER BY id
      LIMIT $3`,
    [opts.sourceId ?? null, afterId, limit, [...AUDIT_ROW_SOURCES]],
  );
...
async function _updateFactEmbeddingsOnce(
  ...
  // PGLite's facts column is always vector(N) (this module casts ::vector
  // everywhere); facts has no updated_at column, embedded_at is the stamp.
