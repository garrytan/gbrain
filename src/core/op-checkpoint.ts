...
export async function recordCompleted(
  ...
  // Pass the RAW array — `executeRawDirect` shares the same `.unsafe()` body as
  // `executeRaw` (runUnsafe), so `JSON.stringify(x)` into `$3::jsonb` double-encodes
  // into a jsonb *string* scalar (PGLite hides it; verified on Postgres), which is
  // why the read path needs `jsonb_typeof = 'array'` corruption handling. The raw
  // array serializes to a proper jsonb array. Sync uses `appendCompleted`, never this.
