...
export const openrouter: Recipe = {
  ...
  touchpoints: {
    ...
    reranker: {
      ...
      // Mixed billing catalog: Cohere bills per search, Voyage per token, and
      // NVIDIA is free, so there is no touchpoint-wide token rate. Every
      // allowlisted model carries an explicit per-model price instead.
      billing_unit: 'mixed',
      model_cost_per_1m_tokens_usd: {
        // Cohere bills per-search, not per-token. These are the pre-existing
        // pseudo-per-1M rates for the budget tracker's heuristic (estimates
        // tokens as chars/4): at ~4K chars/search the tracker estimates
        // ~$0.00025 — in the right ballpark for the per-search bill. Kept
        // unchanged so `--max-cost` installs on the default model behave
        // exactly as before.
