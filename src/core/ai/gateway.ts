...
export async function expand(query: string): Promise<string[]> {
  ...
  try {
    ...
    if (recipe.implementation === 'claude-cli' || recipe.implementation === 'codex-cli') {
      // The CLI subprocess recipes are NOT structured-output capable, despite
      // being 'native' tier. (codex-cli shares the transport shape exactly:
      // rendered prompt → `codex exec` → text.)
