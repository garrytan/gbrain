...
// v0.33.2+: reranker default. Used only when search.reranker.enabled is set
...
/**
 * v0.33.2+: same defense pattern as Voyage's cap but tagged separately so the
...
export function configureGateway(config: AIGatewayConfig): void {
  _config = {
    ...
    // v0.33.2+: reranker_model stays undefined when unset — reranker is
...
/**
 * v0.33.2+: configured reranker model. Returns undefined when no reranker
...
/**
 * v0.33.2+: embed a single text on the QUERY side of an asymmetric retrieval
