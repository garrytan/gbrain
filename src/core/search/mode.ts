...
//
// bump 18→21: `amk=` (autocut minKeep floor) joins the key. The floor changes
// how many rows survive the cut, so a minKeep=1 write (trimmed to the cliff)
// must NOT be served to a minKeep=6 lookup (expects the floor) — same
// contamination class as ac=/acj=/acm=. 16 (det=), 17 (degradation stamp)
// and 18 (acm=, the weak-top floor) were claimed upstream while this PR was
// open, and 19/20 are claimed by the sibling #3617/#3584, so this takes the
// next free number. Same one-time global cold-miss pattern as the bumps above.
...
export function knobsHash(
  ...
  const parts = [
    ...
    // v=21 addition (append-only): autocut minKeep floor (`amk=` — `acm=` above
    // is the v=18 weak-top floor). Changing the floor
    // changes how many rows survive the cut, so a write under one floor must
    // not serve a lookup under another — same contamination class as ac=/acj=.
    // `?? 1` mirrors the defensive read of acj= above for partial-knobs callers.
    `amk=${Math.max(1, Math.floor(knobs.autocut_min_keep ?? 1))}`,
  ];
...
export function loadOverridesFromConfig(
  ...
  const amk = get('search.autocut_min_keep');
  if (amk !== undefined) {
    const n = parseInt(amk, 10);
