...
//
// bump 18→19: `kof=` (keyword AND→OR fallback knob) joins the key. 16 (det=),
// 17 (degradation stamp) and 18 (acm=) were claimed upstream while this PR
// was open, so it takes the next free number. Same one-time global cold-miss
// pattern as the bumps above.
...
export function knobsHash(
  ...
  const parts = [
    ...
    // v=19 addition (append-only): keyword AND→OR fallback knob. A
