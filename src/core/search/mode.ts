...
// cache.ttl_seconds (3600s default).
//
// bump 16→17: `kof=` (keyword AND→OR fallback knob) joins the key. 15 was
// taken by the merged #3677 fts= fold and 16 by the sibling #3621 acm= floor,
// so this PR claims the next free number. Same one-time global cold-miss
// pattern as the bumps above.
...
export function knobsHash(
  ...
  const parts = [
    ...
    `fts=${getFtsLanguage()}`,
    // v=17 addition (append-only): keyword AND→OR fallback knob. A
