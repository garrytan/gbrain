 ...
 *
 * Closes the remaining durability hole in the real-time facts pipeline.
 * Current one-shot CLI writes enqueue a durable `facts-absorb` Minion job,
 * but older releases and interrupted inline callers can leave pages with a
 * failed `facts:absorb` row and no successful retry. The page itself remains
 * durable and searchable; this phase treats those failure rows as a durable
 * recovery backlog. See facts/absorb-log.ts for the ingest-log contract.
 *
