...
export function computeRecommendations(
  ...
  // ---------------------------------------------------------------------
  // extract.stale — materialize DB-backed links and timeline entries.
  // stale_pages is an extraction watermark, not a filesystem sync signal.
  // ---------------------------------------------------------------------
  if (health.stale_pages > 0) {
    const params: Record<string, unknown> = { stale: true };
    ...
    out.push({
      ...
      est_seconds: Math.min(1800, 5 + health.stale_pages * 0.02),
      est_usd_cost: 0,
      depends_on: [],
      rationale: `${health.stale_pages} page${health.stale_pages === 1 ? '' : 's'} awaiting DB link/timeline extraction`,
      status: 'remediable',
    });
  }
