/**
 * X/Twitter tweet fetcher via Playwright GraphQL interception.
 *
 * Ported from personal-knowledge-base/src/x/fetchTweet.mjs.
 * Intercepts X's internal TweetResultByRestId GraphQL endpoint,
 * falls back to DOM scraping when interception fails.
 */
...
export const AUTH_PATH_DEFAULT = (() => {
  const home = process.env.HOME || process.env.USERPROFILE || '~';
  return `${home}/.gbrain/x-auth.json`;
})();
...
export function unwrapResult(result: any): any {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function collectUrls(entities: any): TweetUrl[] {
  return (entities?.urls ?? []).map((entry: { url?: string; expanded_url?: string; display_url?: string }) => ({
    url: entry.url ?? null,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function collectMedia(legacyTweet: any): TweetMedia[] {
  return (legacyTweet?.extended_entities?.media ?? legacyTweet?.entities?.media ?? []).map(
    (media: { type?: string; media_url_https?: string; expanded_url?: string; display_url?: string }) => ({
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function normalizeTweetResult(rawResult: any, sourceUrl: string): NormalizedTweet | null {
...
export async function createXBrowserSession(opts: {
  ...
  const context = await browser.newContext({
    ...
    storageState: opts.storageStatePath || undefined,
  });
...
export async function fetchTweetByUrl(
  ...
  const pageState: PageState = await page
    .evaluate(() => {
      const visibleTweets = Array.from(
        ...
        return { href, statusId, handle, text };
      });
    ...
    .catch(() => ({
      ...
      visibleTweets: [] as PageState['visibleTweets'],
    }));

  const normalized: NormalizedTweet | null =
