...
export const WRITER_NEXT_ACTIONS: Record<string, string> = {
  ...
  publication_not_started: 'Keep the same request_id. Inspect the designated owner and its publication lock; no canonical bytes were replaced, so do not submit a replacement write.',
  database_contention: 'Keep the same request_id; the owner will retry after the SQL lock or connection contention clears.',
  database_unavailable: 'Keep the same request_id and restore database connectivity on the designated owner, then inspect the durable receipt before retrying.',
  preparation_deadline: 'Keep the same request_id. The owner stopped this preparation before publication; inspect provider or database latency, then let the resident writer retry it.',
