...
interface GBrainOAuthProviderOptions {
   ...
   * Requests above it clamp down. Default DEFAULT_DCR_TTL_MAX_SECONDS (7 days).
   */
...
/**
 * Default clamp window for DCR-requested token TTLs (#2179). Admins override
 * via the `oauth.dcr_ttl_min_seconds` / `oauth.dcr_ttl_max_seconds` config
 * keys, read once by `gbrain serve --http` at startup.
 */
export const DEFAULT_DCR_TTL_MIN_SECONDS = 300; // 5 minutes
export const DEFAULT_DCR_TTL_MAX_SECONDS = 7 * 24 * 3600; // 7 days

/**
 * Clamp a DCR-requested token TTL into the admin-configured [min, max]
 * window. Fail-safe by design (#2179): out-of-range values clamp to the
 * nearest bound — registration is never rejected over a TTL hint. Non-integer
 * requests floor; an inverted window collapses to the min bound.
 */
export function clampDcrTokenTtl(
  requested: number,
  min: number = DEFAULT_DCR_TTL_MIN_SECONDS,
  max: number = DEFAULT_DCR_TTL_MAX_SECONDS,
): number {
...
class GBrainClientsStore implements OAuthRegisteredClientsStore {
  constructor(
    ...
    private allowClientCredentialsDcr = false,
    private dcrTtlMin: number = DEFAULT_DCR_TTL_MIN_SECONDS,
    private dcrTtlMax: number = DEFAULT_DCR_TTL_MAX_SECONDS,
  ) {}
...
export class GBrainOAuthProvider implements OAuthServerProvider {
  ...
  constructor(options: GBrainOAuthProviderOptions) {
    ...
    this._clientsStore = new GBrainClientsStore(
      ...
      options.dcrTtlMinSeconds ?? DEFAULT_DCR_TTL_MIN_SECONDS,
      options.dcrTtlMaxSeconds ?? DEFAULT_DCR_TTL_MAX_SECONDS,
    );
