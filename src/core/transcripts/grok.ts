 ...
 * prompt_history.jsonl, …) are not sessions — discovery and glob expansion
 * skip them the way OpenClaw checkpoint siblings are skipped.
 *
 ...
 * prompt_history.jsonl. Glob expansion and discovery skip these so a
 * `~/.grok/sessions/**` pass does not treat thousands of updates.jsonl /
 * summary.json files as sessions (or as host-format drift).
 */
