...
export type CyclePhase =
  ...
  // Durable recovery for the real-time facts pipeline: re-runs extraction
  // (inline) on pages with an unresolved facts:absorb failure in ingest_log.
  // Default ON, bounded by page cap + cost cap + walltime deadline.
  | 'realtime_absorb_recovery'
...
export const ALL_PHASES: CyclePhase[] = [
  ...
  'conversation_facts_backfill',
  // Real-time absorb recovery: after the backfill, before embed, so
  // recovered facts get embedded same-cycle. Default ON (bounded by page
  // cap + cost cap + walltime deadline inside the wrapper).
  'realtime_absorb_recovery',
...
const NEEDS_LOCK_PHASES: ReadonlySet<CyclePhase> = new Set([
  ...
  'conversation_facts_backfill',
  // Inserts facts (inline) + appends recovery tombstone rows; needs lock.
  'realtime_absorb_recovery',
...
export interface CycleOpts {
  ...
  /**
   * Optional progress sink supplied by a long-lived caller. Minion handlers
   * use this to persist the currently running phase on the job row so a
   * timeout names the exact phase that was interrupted.
   */
...
export async function runCycle(
  ...
  try {
    ...
    // inline on pages with an unresolved facts:absorb failure in ingest_log.
    // Default ON; bounded by page cap + cost cap + walltime deadline.
    if (phases.includes('realtime_absorb_recovery')) {
      checkAborted(opts.signal);
      if (!engine) {
        phaseResults.push({
        ...
        progress.start('cycle.realtime_absorb_recovery');
        const { runPhaseRealtimeAbsorbRecovery } = await import('./cycle/realtime-absorb-recovery.ts');
        const { result, duration_ms } = await timePhase(() =>
          runPhaseRealtimeAbsorbRecovery(engine, { dryRun, signal: opts.signal }),
        );
        result.duration_ms = duration_ms;
        phaseResults.push(result);
        progress.finish();
