...
interface CanonicalTargetProposalMaintenanceCounts {
  created: number;
  existing: number;
  blocked: number;
  hard_blocked: number;
...
async function generateCanonicalTargetProposals(
  engine: BrainEngine,
  candidates: MemoryCandidateEntry[],
  scopeId: string,
): Promise<CanonicalTargetProposalMaintenanceCounts> {
  const counts = emptyCanonicalTargetProposalCounts();
  for (const candidate of sortCandidates(candidates)) {
    try {
      const result = await createCanonicalTargetProposal(engine, {
      ...
      if (result.kind === 'created' && result.proposal.status !== 'blocked') {
        counts.created += 1;
      }
