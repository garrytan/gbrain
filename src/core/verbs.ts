...
const synthesize: Operation = {
  ...
  params: {
    ...
    pages_budget_chars: { type: 'number', description: 'Total character budget for the gathered-pages evidence block (default 12000; overrides `synthesize.pages_budget_chars` config). Warnings flag truncated excerpts — read full pages before quoting.' },
  },
  scope: 'read',
  verb: true,
  ...
  handler: async (ctx, p) => {
    ...
    const result = await runThink(ctx.engine, {
      ...
      ...(typeof p.pages_budget_chars === 'number' ? { pagesBudgetChars: p.pages_budget_chars } : {}),
