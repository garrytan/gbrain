...
export interface ThinkGatherOpts {
  ...
  /** Federated/source scope (array wins over scalar, matching sourceScopeOpts).
   *  Threaded into every retriever so think honors the caller's grant like the
   *  direct reads do. Omitted (local CLI) = no source filter. */
  sourceId?: string;
  sourceIds?: string[];
}
...
export async function runGather(
  ...
  // #2200-class: thread the caller's source/federated scope into every retriever
  // (array wins over scalar) so think can't read pages/takes/graph outside the
  // granted sources. Empty object = no filter (local CLI).
  const sourceScope =
