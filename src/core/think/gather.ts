...
/**
 * Read a positive-integer tuning knob from the environment, falling back to
 * the upstream default. Lets retrieval breadth be tuned per-install (e.g. a
 * brain with many near-duplicate sources that needs wider conflict/variant
 * recall) without changing default behavior. Matches gbrain's GBRAIN_* env
 * knob pattern (env above config, incident escape hatch).
 */
