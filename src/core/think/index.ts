...
export async function runThink(
  ...
  // Restored from master after a MEMORY_VERBS / truthful-surface merge dropped
  // it from the return path. Failed compose (empty_answer / not_json / …)
  // keeps its typed status + extractive fallback — we still drop unsupported
  // citations, but we do not overwrite a failed-synthesis shape with the
  // refuse template (that would flip synthesisOk and hide the real failure).
  ...
  const canRefuseCompose = synthesisStatus === 'ok' && response.answer.trim().length > 0;
  if (shouldRefuse && canRefuseCompose) {
