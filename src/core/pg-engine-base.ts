...
import type {
  ...
  DerivedJob, DerivedJobFailureInput, DerivedJobFilters, DerivedJobInput, DerivedJobLeaseInput, DerivedJobLeaseReleaseInput,
  NoteManifestEntry, NoteManifestEntryInput, NoteManifestFilters,
  NoteSectionEntry, NoteSectionEntryInput, NoteSectionFilters,
  Chunk, ChunkInput,
  GraphNode,
  IngestLogEntry, IngestLogInput,
  Link,
  MemoryCandidateContradictionEntry, MemoryCandidateContradictionEntryInput,
  ...
  MemoryRedactionPlan, MemoryRedactionPlanFilters, MemoryRedactionPlanInput,
  ...
  TaskWorkingSet, TaskWorkingSetInput,
...
/**
 * Minimal query surface shared by the two Postgres-dialect transports:
 * postgres.js (`sql.unsafe`) and PGlite (`db.query`). All shared SQL logic in
 * PgEngineBase is written against this seam so each engine only supplies the
 * transport.
 *
 * Deliberately NOT named `sql` or `db`: external modules (migrate.ts,
 * lifecycle-forgetting.ts, assertion stores) duck-type engines via
 * `'sql' in engine` / `'db' in engine` to pick a transport-specific path.
 */
...
/**
 * Shared SQL implementation for PostgresEngine and PGLiteEngine (F7).
 *
 * Subclasses own the transport only: connection lifecycle, schema init,
 * `queryable`, `transaction`, `withSearchTimeout`, and `execRaw`. Every method
 * here must behave identically on both engines; engine-specific behavior
 * (e.g. PGlite's inline page-embedding refresh) hangs off protected hooks.
 */
export abstract class PgEngineBase {
  /** Transport seam: issue a parameterized query and return its rows. */
  protected abstract get queryable(): PgQueryable;

  abstract transaction<T>(fn: (engine: BrainEngine) => Promise<T>): Promise<T>;

  /**
   * Run a search callback against a transport that may enforce a statement
   * timeout (postgres reserves a connection and sets statement_timeout;
   * PGlite runs the callback directly).
   */
  protected abstract withSearchTimeout<T>(fn: (q: PgQueryable) => Promise<T>): Promise<T>;

  /** Execute raw (possibly multi-statement) SQL, e.g. migration scripts. */
  protected abstract execRaw(sql: string): Promise<void>;

  /** Resolve the transport seam of a transaction-scoped engine handle. */
  protected txq(engine: BrainEngine): PgQueryable {
    return (engine as unknown as PgEngineBase).queryable;
  }
  ...
  async getPageProjection(slug: string, options: PageProjectionOptions = {}): Promise<PageProjection | null> {
    return this.getPageProjectionWith(this.queryable, slug, options);
  }

  protected async getPageProjectionWith(
    q: PgQueryable,
    slug: string,
  ...
  async deletePage(slug: string): Promise<void> {
    ...
    await this.transaction(async (txBase) => {
      ...
      await q.query(`LOCK TABLE pages IN SHARE ROW EXCLUSIVE MODE`);
      await q.query('SELECT id FROM pages WHERE slug = $1 FOR UPDATE', [normalizedSlug]);
      await q.query(`LOCK TABLE derived_jobs IN EXCLUSIVE MODE`);
      await q.query('DELETE FROM derived_jobs WHERE slug = $1', [normalizedSlug]);
      await q.query('DELETE FROM derived_index_state WHERE slug = $1', [normalizedSlug]);
      await q.query('DELETE FROM pages WHERE slug = $1', [normalizedSlug]);
    });
  }
  ...
  async listPages(filters?: PageFilters): Promise<Page[]> {
    ...
    if (filters?.type && filters?.tag) {
      result = await this.queryable.query(
  ...
  async resolveSlugs(partial: string): Promise<string[]> {
    ...
    const exact = await this.queryable.query('SELECT slug FROM pages WHERE slug = $1', [partial]);
  ...
  async deleteChunks(slug: string): Promise<void> {
    const { rows } = await this.queryable.query('SELECT id FROM pages WHERE slug = $1', [slug]);
  ...
  async getPageEmbeddings(type?: PageType): Promise<Array<{
    ...
    const { rows } = await this.queryable.query(query, params);
  ...
  /**
   * Hook invoked after chunk rows for a page are inserted, updated, or
   * deleted. PGlite refreshes the page-embedding centroid inline here;
   * Postgres leaves embedding refresh to its async embedding pipeline.
   */
  protected async onChunksChanged(_pageId: number): Promise<void> {}
  ...
  async getStats(): Promise<BrainStats> {
    const { rows: [stats] } = await this.queryable.query(`
      SELECT
    ...
    const { rows: types } = await this.queryable.query(
  ...
  async listMemoryCandidateStatusEventsByInteractionIds(
    ...
    // below, so they can run concurrently (PGlite serializes internally).
    const batches = await Promise.all(chunkInteractionIds(uniqueInteractionIds).map(async (chunk) => {
  ...
  async updateMemoryRedactionPlanItemStatus(
    ...
    const currentRows = await this.queryable.query(
      `SELECT id, plan_id, target_object_type, target_object_id, field_path,
              before_hash, after_hash, status, preview_text, created_at, updated_at
       FROM memory_redaction_plan_items
       WHERE id = $1`,
  ...
  async supersedeMemoryCandidateEntry(
    ...
    try {
      return await this.transaction(async (txBase) => {
        const txQ = this.txq(txBase);
        const { rows: supersededRows } = await txQ.query(
        ...
        const { rows: replacementRows } = await txQ.query(
          `SELECT id, scope_id, status
           FROM memory_candidate_entries
           WHERE id = $1
           FOR UPDATE`,
        ...
        const { rows: updatedRows } = await txQ.query(
          `UPDATE memory_candidate_entries
  ...
  async listMemoryCandidateSupersessionEntriesByInteractionIds(
    ...
    // Batches are independent read-only queries and the result is re-sorted
    // below, so they can run concurrently (PGlite serializes internally).
    const batches = await Promise.all(chunkInteractionIds(interactionIds).map(async (chunk) => {
      const placeholders = chunk.map((_, index) => `$${index + 1}`).join(', ');
      const { rows } = await this.queryable.query(
        `SELECT id, scope_id, superseded_candidate_id, replacement_candidate_id,
                reviewed_at, review_reason, interaction_id, created_at, updated_at
  ...
  async listMemoryCandidateContradictionEntriesForCandidateIds(
    ...
    // Batches are independent read-only queries and the result is re-sorted
    // below, so they can run concurrently (PGlite serializes internally).
    const batches = await Promise.all(chunkInteractionIds(candidateIds).map(async (chunk) => {
      const placeholders = chunk.map((_, index) => `$${index + 1}`).join(', ');
      const { rows } = await this.queryable.query(
        `SELECT id, scope_id, candidate_id, challenged_candidate_id, outcome, supersession_entry_id,
                reviewed_at, review_reason, interaction_id, created_at, updated_at
  ...
  async deleteMemoryCandidateEntry(id: string): Promise<void> {
    await this.queryable.query(`DELETE FROM memory_candidate_entries WHERE id = $1`, [id]);
  }
    ...
    const { rows } = await this.queryable.query(
      `UPDATE derived_jobs
       ...
       RETURNING id, scope_id, slug, artifact_kind, target_content_hash, manifest_path,
                 derived_parameters, status, attempts, last_error, lease_owner,
                 lease_expires_at, created_at, updated_at`,
      [nextStatus, input.error, input.id],
    );
    if (rows.length === 0) return this.getDerivedJobById(input.id);
  ...
  private async enqueueDerivedJobInTransaction(input: DerivedJobInput): Promise<DerivedJob> {
    ...
    const active = await this.queryable.query(
      `SELECT id, scope_id, slug, artifact_kind, target_content_hash, manifest_path,
              derived_parameters, status, attempts, last_error, lease_owner,
              lease_expires_at, created_at, updated_at
       FROM derived_jobs
  ...
  async listDerivedIndexStates(filters?: DerivedIndexStateFilters): Promise<DerivedIndexState[]> {
    return this.listDerivedIndexStatesWith(this.queryable, filters);
  }

  protected async listDerivedIndexStatesWith(
    q: PgQueryable,
  ...
  private async markDerivedIndexReadyInTransaction(input: {
    ...
    if (matchingJob) {
      await this.queryable.query(`DELETE FROM derived_jobs WHERE id = $1`, [matchingJob.id]);
    }
  ...
  private async upsertFailedDerivedIndexStateFromJob(
    ...
    await this.queryable.query(
      `INSERT INTO derived_index_state (
        scope_id, slug, artifact_kind, target_content_hash, indexed_content_hash,
        status, extractor_version, derived_schema_version, last_error, updated_at
      ) VALUES ($1, $2, $3, $4, NULL, 'failed', $5, $6, $7, now())
      ON CONFLICT (scope_id, slug, artifact_kind) DO UPDATE SET
        target_content_hash = EXCLUDED.target_content_hash,
        indexed_content_hash = NULL,
        status = EXCLUDED.status,
  ...
  async deleteContextMapEntry(id: string): Promise<void> {
    await this.queryable.query(`DELETE FROM context_map_entries WHERE id = $1`, [id]);
  }
  ...
  async deleteContextAtlasEntry(id: string): Promise<void> {
    await this.queryable.query(`DELETE FROM context_atlas_entries WHERE id = $1`, [id]);
  }
  ...
  async getConfig(key: string): Promise<string | null> {
    const { rows } = await this.queryable.query('SELECT value FROM config WHERE key = $1', [key]);
  ...
  async searchVector(embedding: Float32Array, opts?: SearchOpts): Promise<SearchResult[]> {
    ...
    return this.withSearchTimeout(async (q) => {
      ...
      return appendPendingDerivedSearchResults(
        {
          listDerivedIndexStates: (filters) => this.listDerivedIndexStatesWith(q, filters),
          getPageProjection: (slug, options) => this.getPageProjectionWith(q, slug, options),
        },
        (rows as Record<string, unknown>[]).map((row) => rowToSearchResult(row)),
        opts,
      );
    });
  }
