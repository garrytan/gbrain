...
function evaluate(): ConversionEvidenceObligation[] {
  ...
  const completeCandidates = { images: 1, tables: 0, equations: 0, charts: 0 };
  const exactVerdict = deriveNormalizedVerdict({ sourceVisualKind: 'image', coverage: { failed: 1 }, unreadableRegions: [], warnings: [], candidates: completeCandidates, confidence: null, ocr: { state: 'success', confidence: 1, textExtracted: true }, imageDimensions: { width: 1, height: 1 } });
  ...
  add('multimodal-observation.zero-candidate-neutral', 'multimodal-observation', deriveNormalizedVerdict({ sourceVisualKind: 'image', coverage: { failed: 0 }, unreadableRegions: [], warnings: [], candidates: { images: 0, tables: 0, equations: 0, charts: 0 }, confidence: null, ocr: { state: 'success', confidence: 1, textExtracted: true }, imageDimensions: { width: 1, height: 1 } }).reasonCodes.length === 0);
