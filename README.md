...
Durability is the point: every subagent turn commits to `subagent_messages`, every tool call to `subagent_tool_executions`. Worker kills, OpenClaw crashes, timeouts — all resumable. Host repos (your OpenClaw, etc.) ship their own subagent definitions via `GBRAIN_PLUGIN_PATH` + a `gbrain.plugin.json` manifest: see [`docs/guides/plugin-authors.md`](docs/guides/plugin-authors.md). Default provider is `anthropic-sdk` (requires `ANTHROPIC_API_KEY`); set `GBRAIN_SUBAGENT_PROVIDER=claude-cli` to use local Claude CLI auth for no-tool subagent turns.
