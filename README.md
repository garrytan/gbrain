...
- **Chat / model providers**: any gateway recipe drives the chat + expansion touchpoints (`think`, `dream`, subagents) — Anthropic, OpenAI, and friends via API key. The `openai-codex` recipe instead authenticates with a ChatGPT-plan OAuth login (no `OPENAI_API_KEY`): browser PKCE login, keychain token store, dynamic model discovery. Setup walkthrough in [`docs/ai-providers/openai-codex.md`](docs/ai-providers/openai-codex.md).
