...
PMBrain 的设计系统源文档。最初源于 v0.26.0 管理后台 SPA 工作期间落在 `admin/src/index.css` 中的事实代币，并随桌面端与管理台持续对齐。
...
| 代币               | 浅色值      | 深色值      | 用途                         |
|--------------------|-------------|-------------|------------------------------|
| `--bg-primary`     | `#f6f8fb`   | `#0d1218`   | 页面背景                     |
| `--bg-secondary`   | `#ffffff`   | `#141b24`   | 侧边栏、卡片                 |
| `--bg-tertiary`    | `#eef2f7`   | `#1c2531`   | 次级表面、悬停状态           |
| `--text-primary`   | `#121826`   | `#e8eef6`   | 正文文本                     |
| `--text-secondary` | `#5d6678`   | `#a9b4c3`   | 标题、标签                   |
| `--text-muted`     | `#8a93a5`   | `#7f8b9d`   | 三级文本                     |
| `--accent`         | `#355cff`   | `#8e8bff`   | 活动状态、链接、主要行动按钮 |
| `--success`        | `#119764`   | `#57c6ab`   | 健康 / 正常状态              |
| `--warning`        | `#b7791f`   | `#e2aa5c`   | 需要注意的状态               |
| `--error`          | `#dc2626`   | `#ff7f7f`   | 失败、破坏性确认             |

管理台和桌面端默认跟随操作系统主题，并允许用户固定为浅色或深色。管理台选择保存在当前浏览器，桌面端选择保存在本机配置；`system` 必须同时响应系统运行时主题变化。

WCAG 对比度需要分别在浅色和深色主题下检查；正文、表单标签和可操作文字至少满足 AA。颜色不能作为状态的唯一表达。
...
- 桌面宽度使用 260px 左侧边栏。活动项目在 `--accent` 中获得 3px 左边框。
- 768px 以下隐藏侧边栏，改用顶部页面选择器；数据表在手机端减少次要列或使用局部横向滚动。
- 主内容区域使用剩余宽度。
