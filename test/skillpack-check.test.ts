/**
 * Tests for `cortex skillpack-check` — the agent-readable health report.
 *
 * Covers:
 *   - Healthy fresh install → exit 0, healthy:true, actions:[], no DB needed.
 *   - Half-migrated (partial entry in completed.jsonl) → exit 1,
 *     healthy:false, actions includes `cortex apply-migrations --yes`,
 ...
 * state. DATABASE_URL / CORTEX_DATABASE_URL / legacy aliases stripped so the report runs
...
function run(args: string[]): { exitCode: number; stdout: string; stderr: string } {
  ...
  delete env.CORTEX_DATABASE_URL;
...
beforeEach(() => {
  ...
  tmp = mkdtempSync(join(tmpdir(), 'cortex-skillpack-check-test-'));
});
...
describe('cortex skillpack-check', () => {
  test('healthy fresh install → exit 0, healthy:true, empty actions', () => {
    ...
    expect(report.summary).toBe('cortex skillpack healthy');
  ...
  test('half-migrated (partial completed.jsonl) → exit 1, apply-migrations in actions', () => {
    ...
    expect(report.actions).toContain('cortex apply-migrations --yes');
    expect(report.summary).toContain('cortex apply-migrations --yes');
