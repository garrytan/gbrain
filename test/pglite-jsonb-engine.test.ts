/**
 * PGlite JSONB regression coverage — mirrors test/postgres-jsonb-engine.test.ts.
 *
 * The PR #176 engine dedupe moved all SQL into PgEngineBase, so a serialization
 * regression in the shared base (or a PGlite driver behavior change) would
 * stringify JSONB writes into scalar strings. The Postgres variant of this test
 * caught exactly that class of bug; this keeps the PGlite side honest without
 * needing DATABASE_URL, so it runs in every unit-test environment.
 */
...
async function createEngine(): Promise<PGLiteEngine> {
...
async function queryRows(
  ...
  const result = await engine.db.query<Record<string, unknown>>(text, params);
  return result.rows;
}

test('pglite stores page frontmatter, raw data, and note manifest JSON columns as structured JSON', async () => {
  const engine = await createEngine();
  const slug = `systems/pglite-jsonb-${crypto.randomUUID()}`;
  const scopeId = `workspace:jsonb:${crypto.randomUUID()}`;

  try {
    const page = await engine.putPage(slug, {
      type: 'system',
      title: 'PGlite JSONB Regression',
    ...
    const pageKinds = await queryRows(
      engine,
      'SELECT jsonb_typeof(frontmatter) AS frontmatter_kind FROM pages WHERE slug = $1',
      [slug],
    );
    expect(pageKinds[0]?.frontmatter_kind).toBe('object');

    const rawKinds = await queryRows(
      engine,
      `SELECT jsonb_typeof(rd.data) AS data_kind
       FROM raw_data rd
       JOIN pages p ON p.id = rd.page_id
       WHERE p.slug = $1 AND rd.source = 'jsonb-regression'`,
      [slug],
    );
    expect(rawKinds[0]?.data_kind).toBe('object');

    const manifestKinds = await queryRows(
      engine,
      `SELECT
...
test('pglite stores task-memory and retrieval-trace JSON columns as structured JSON', async () => {
  const engine = await createEngine();
  ...
  try {
    await engine.createTaskThread({
      ...
      status: 'active',
      repo_path: '/repo',
      branch_name: 'pglite-jsonb-correctness',
      current_summary: 'Need structured JSONB writes',
    });

    await engine.upsertTaskWorkingSet({
      task_id: taskId,
      active_paths: ['src/core/pglite-engine.ts'],
      active_symbols: ['PGLiteEngine'],
    ...
    await engine.recordTaskAttempt({
      ...
      outcome: 'failed',
      applicability_context: { engine: 'pglite' },
    ...
    await engine.recordTaskDecision({
      id: `decision-${taskId}`,
      task_id: taskId,
      summary: 'Keep JSONB writes structured.',
      rationale: 'The shared PG base must encode structured JSON directly.',
    ...
    const workingSetKinds = await queryRows(
      engine,
      `SELECT
    ...
    const attemptKinds = await queryRows(
      engine,
      `SELECT
    ...
    const decisionKinds = await queryRows(
      engine,
      `SELECT
    ...
    const traceKinds = await queryRows(
      engine,
      `SELECT
...
test('pglite stores context map and context atlas JSON columns as structured JSON', async () => {
  const engine = await createEngine();
  ...
  try {
    ...
    const mapKinds = await queryRows(
      engine,
      'SELECT jsonb_typeof(graph_json) AS graph_kind FROM context_map_entries WHERE id = $1',
      [mapId],
    );
    expect(mapKinds[0]?.graph_kind).toBe('object');

    const atlasKinds = await queryRows(
      engine,
      'SELECT jsonb_typeof(entrypoints) AS entrypoints_kind FROM context_atlas_entries WHERE id = $1',
      [atlasId],
    );
...
// NOTE: like its Postgres sibling, this test rewinds the version pointer on a
// fully-migrated database and replays migrations 22..latest. That is only
// valid while every migration in that range stays idempotent (IF NOT EXISTS /
// CREATE OR REPLACE / guarded DO blocks) — the repo-wide invariant the
// migration suite already relies on for crash recovery. If a future migration
// breaks idempotency, fix the migration, not this test.
test('pglite migrations repair legacy JSONB scalar strings before JSONB operators run', async () => {
  const engine = await createEngine();
  ...
  try {
    await queryRows(
      ...
      [candidateId, JSON.stringify(['legacy:source'])],
    );

    const before = await queryRows(
      engine,
      'SELECT jsonb_typeof(source_refs) AS source_refs_kind FROM memory_candidate_entries WHERE id = $1',
      [candidateId],
    );
