 ...
 *
 * Reads the doctor source surfaces via a literal-string scan, enumerates every
 * `name: '<...>'` Check name, and asserts each appears in exactly ONE category
 * set. The union of the four sets must equal the discovered names exactly — no
 * orphans, no extras.
 *
...
describe('doctor-categories drift guard', () => {
  ...
  test('every categorized name is currently used in doctor surfaces (no stale entries)', () => {
    const discovered = enumerateCheckNames();
    const allCategorized = new Set<string>([
      ...BRAIN_CHECK_NAMES,
      ...SKILL_CHECK_NAMES,
    ...
    if (stale.length > 2) {
      throw new Error(
        `These categorized names no longer appear in doctor surfaces: ${stale.sort().join(', ')}. ` +
