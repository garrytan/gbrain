 ...
 *
 * Reads doctor-owned check sources via a literal-string scan, enumerates every
...
describe('doctor-categories drift guard', () => {
  ...
  test('every categorized name is currently used in doctor.ts source (no stale entries)', () => {
    ...
    if (stale.length > 2) {
      throw new Error(
        `These categorized names no longer appear in doctor check sources: ${stale.sort().join(', ')}. ` +
