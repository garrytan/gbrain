...
describe('KNOWN_CONFIG_KEYS', () => {
  ...
  test('registers cycle.timezone for local-day dream bucketing', () => {
...
describe('#2753 — the doctor-proposed gateway-loop command is accepted by `config set`', () => {
  ...
  test('cycle.timezone: rejects an invalid IANA timezone before writing', async () => {
  ...
  test('cycle.timezone: accepts a valid IANA timezone', async () => {
    const { engine, setCalls } = setStubEngine();
    const { errs, exit } = await runConfigCapture(
      engine,
