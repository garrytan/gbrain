...
function basisEmbedding(index: number): Float32Array {
  const embedding = new Float32Array(DIM);
  embedding[index] = 1;
  return embedding;
}

async function seedTypedLookupCollision(): Promise<void> {
  await engine.putPage('people/jordan-example', {
    type: 'person',
    title: 'Jordan Example',
    compiled_truth: 'A canonical synthetic profile with unrelated body text.',
  });
  await engine.upsertChunks('people/jordan-example', [
    {
      chunk_index: 0,
      chunk_text: 'A canonical synthetic profile with unrelated body text.',
      chunk_source: 'compiled_truth',
      // Deliberately no embedding: this page is generated by the title arm,
      // while the distractor below wins both body-keyword and vector arms.
    },
  ]);

  await engine.putPage('people/morgan-example', {
    type: 'person',
    title: 'Morgan Example',
    compiled_truth: 'Jordan Example appears repeatedly in this synthetic relationship note.',
  });
  await engine.upsertChunks('people/morgan-example', [
    {
      chunk_index: 0,
      chunk_text: 'Jordan Example appears repeatedly in this synthetic relationship note.',
      chunk_source: 'compiled_truth',
...
describe('hybridSearch wiring — title arm reaches the fused result set', () => {
  ...
  test('typed person lookup hard-prefers the exact-title page over a stronger body/vector distractor', async () => {
    await seedTypedLookupCollision();

    const results = await hybridSearch(engine, 'Jordan Example', {
      limit: 5,
      types: ['person'],
  ...
  test('broad untyped body query keeps semantic ordering', async () => {
    await seedTypedLookupCollision();

    const results = await hybridSearch(engine, 'synthetic relationship note', {
      limit: 5,
      queryEmbedFn: () => basisEmbedding(0),
    });

    expect(results[0].slug).toBe('people/morgan-example');
  });
});
