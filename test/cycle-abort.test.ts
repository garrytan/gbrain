...
describe('autopilot-cycle handler contract (v0.20.5)', () => {
  test('handler registration passes signal to runCycle', async () => {
    ...
    //
    // Bound the assertion to the complete handler instead of a fixed-size
    // source window. Source-scope validation legitimately grows this handler;
    // a byte ceiling can fail even while the signal contract remains intact.
    const handlerStart = jobsSource.indexOf("registerBuiltinJob(worker, engine, 'autopilot-cycle'");
    expect(handlerStart).toBeGreaterThan(-1);
    const handlerEnd = jobsSource.indexOf(
      "worker.register('autopilot-global-maintenance'",
      handlerStart,
    );
    expect(handlerEnd).toBeGreaterThan(handlerStart);
    const handlerBlock = jobsSource.slice(handlerStart, handlerEnd);
