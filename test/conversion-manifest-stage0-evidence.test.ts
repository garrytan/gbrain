...
function receipt(): Receipt {
  const path = process.env.CONVERSION_MANIFEST_COMPAT_EVIDENCE;
  return parseReceipt(path ? JSON.parse(readFileSync(resolve(path), 'utf8')) as unknown : fallback);
}

describe('conversion manifest Stage-0 compatibility evidence', () => {
  test('validates the live receipt schema and recomputable SHA without a user path', () => {
    const value = receipt();
    ...
    if (process.env.CONVERSION_MANIFEST_COMPAT_EVIDENCE) { expect(value.sha256).toMatch(/^[0-9a-f]{64}$/); expect(value.sha256).toBe(createHash('sha256').update(canonical(value)).digest('hex')); }
  });
  test('pure verifier fixture proves the canonical hash excludes only top-level sha256', () => {
    const signed = { ...fallback, sha256: createHash('sha256').update(canonical(fallback)).digest('hex') };
    expect(signed.sha256).toBe(createHash('sha256').update(canonical(signed)).digest('hex'));
    expect(createHash('sha256').update(canonical({ ...fallback, created_at: '2026-01-02T00:00:00.000Z' })).digest('hex')).not.toBe(signed.sha256);
  });
});
