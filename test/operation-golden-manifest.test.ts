...
import { buildOperationGoldenManifest, OPERATION_GOLDEN_MANIFEST_SCHEMA_VERSION } from '../src/core/services/operation-conformance-service.ts';
...
describe('operation golden manifest', () => {
  test('builds a deterministic manifest from the live operation registry', () => {
    ...
    expect(manifest.operations.map((entry) => entry.name)).toEqual([...manifest.operations.map((entry) => entry.name)].sort());
