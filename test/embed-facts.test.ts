...
import { AUDIT_ROW_SOURCES, TERMINAL_AUDIT_SOURCE } from '../src/core/facts/audit-sources.ts';
...
async function pendingCount(): Promise<number> {
  ...
  // (#4875: audit rows are not embeddable, so they are excluded here too).
...
describe('embedStaleFacts', () => {
  ...
  test('predicate parity: the literal facts_pending SQL matches listFactsNeedingEmbedding', async () => {
    ...
    // An audit-row fact: present but never embeddable (#4875) — the doctor's
    // facts_pending counter and this selector must both exclude it.
    await engine.insertFacts(
      [{ fact: 'audit checkpoint row', kind: 'fact', source: TERMINAL_AUDIT_SOURCE, row_num: 11, source_markdown_slug: 'people/grace-example' }],
      { source_id: 'default' },
    );
    // Embed one so the fixture spans all four states: pending, embedded,
    // expired, and audit-row.
