...
describe('extractPageLinks', () => {
  ...
  test('person-page role prior biases company refs to invested_in when prose names partner role', () => {
    const candidates = extractPageLinks(
      'Wendy is a partner at Founders Fund. Her portfolio includes [Cipher Labs](companies/cipher-labs).',
      {},
      'person',
    );
    const cipher = candidates.find(c => c.targetSlug === 'companies/cipher-labs');
    expect(cipher?.linkType).toBe('invested_in');
  });
});
...
describe('inferLinkType', () => {
  ...
  test('founder noun-form variants -> founded', () => {
    expect(inferLinkType('person', 'Carol Wilson is the founder of Anchor.')).toBe('founded');
    expect(inferLinkType('person', 'The founders are Alice and Bob.')).toBe('founded');
    expect(inferLinkType('person', 'Dana is one of the founders of Northstar.')).toBe('founded');
  });
