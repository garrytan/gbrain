...
// Recipe metadata is no longer embedded here — features reads the real
// recipes. Coverage of specific recipes is asserted in integrations.test.ts
// (listRecipeConfigStatus), which can see the actual list.
describe('recipe metadata', () => {
  it('module loads without an embedded recipe table', async () => {
    const mod = await import('../src/commands/features.ts');
...
describe('x-to-brain secret name alignment (#2789)', () => {
  ...
  it('features has no secret-name registry of its own to drift', () => {
    const src = read('../src/commands/features.ts');
    // This used to pin the RECIPE_META line for x-to-brain. That table is
    // gone: features now derives from the recipes themselves via
    // listRecipeConfigStatus(), so the features-vs-recipe disagreement this
    // suite was written to catch cannot be expressed any more. The remaining
    // tests below still pin the recipe as the single canonical surface.
    expect(src).not.toContain('const RECIPE_META');
    expect(src).not.toMatch(/secrets:\s*\['[A-Z_]+'\]/);
    expect(src).toContain('listRecipeConfigStatus');
  });

  it('features reaches recipe status through core, never the integrations command', () => {
    // The flag registry is generated by scanning a command module plus ONE
    // level of its relative imports, so importing commands/integrations.ts
    // here would attribute that command's flags to `gbrain features` and let
    // it accept flags it never reads. src/core/recipe-status.ts is the seam
    // that keeps the scan flag-free; see its header.
    const src = read('../src/commands/features.ts');
    expect(src).not.toMatch(/import\('\.\/integrations\.ts'\)/);
    expect(src).not.toMatch(/from\s+'\.\/integrations\.ts'/);
    expect(src).toContain('core/recipe-status.ts');
  });
