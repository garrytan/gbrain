 ...
 *
 * Pins:
 *  - The voyage recipe declares a reranker touchpoint: the rerank-3 pair +
 *    the rerank-2.5 pair, with rerank-2.5 still the default, POST path
 *    '/rerank' (base_url_default already ends in /v1 — no /v1/v1 doubling),
 *    request top-N key `top_k` via `top_param`.
...
describe('voyage reranker touchpoint (v0.46.3)', () => {
  test('declares rerank-2.5 as the default reranker at /rerank with top_k', () => {
    ...
    expect(tp!.models).toEqual(['rerank-3', 'rerank-3-lite', 'rerank-2.5', 'rerank-2.5-lite']);
