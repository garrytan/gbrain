...
describe('buildChecks — orchestrator against PGLite', () => {
  ...
  test('--fast skips DB-dependent checks; filesystem checks still run', async () => {
    // Fast mode performs one lightweight connection probe, then bails out
    // before every expensive DB-dependent check. Filesystem checks above
    // the DB phase still run.
    ...
    // An unconfigured install remains filesystem-only and non-failing.
    const fsOnlyChecks = await buildChecks(null, ['--fast']);
    ...
    expect(conn.message).toContain('No database configured');
  });
