...
describeMaybe('phantom-redirect E2E (Postgres)', () => {
  ...
  test('round 12: postgres-js text-string embedding survives migration', async () => {
    ...
    try {
      ...
      // a clean NULL). Migration v40 uses halfvec on pgvector >= 0.7 and
      // falls back to vector on older installs, so both are valid.
        ...
        `SELECT COUNT(*)::text AS ct FROM facts
         WHERE source_id='default'
           ...
           AND (
             ...
             WHERE oid = pg_typeof(embedding)::oid
           ) NOT IN ('vector', 'halfvec')`,
      );
