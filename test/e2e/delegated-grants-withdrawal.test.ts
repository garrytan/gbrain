...
async function cleanupDelegationFixtures() {
  const engine = getEngine();
  await engine.executeRaw("DELETE FROM minion_jobs WHERE data->>'__owner_client_id'='pg-agent-fixture'");
...
beforeAll(async () => {
  if (!RUN) return;
  await setupDB();
  await cleanupDelegationFixtures();
});
afterAll(async () => {
  if (!RUN) return;
  await cleanupDelegationFixtures();
