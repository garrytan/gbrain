...
describe('gbrain doctor --json subprocess smoke (D10/CMT-2)', () => {
  ...
  test.skipIf(SKIP)('doctor --fast exits nonzero when the configured database is unreachable', async () => {
    const home = mkdtempSync(join(tmpdir(), 'gbrain-doctor-fast-fail-'));
    try {
      const env = {
        HOME: home,
        GBRAIN_HOME: home,
        GBRAIN_DATABASE_URL: 'postgresql://postgres:postgres@127.0.0.1:1/gbrain_test',
        GBRAIN_NO_RETRY_CONNECT: '1',
      };

      const doctor = await runCli(
        ['doctor', '--fast', '--json', '--scope=brain', '--no-retry-connect'],
        env,
        30_000,
      );
      ...
      expect(connection?.message).toContain('env:GBRAIN_DATABASE_URL');
    } finally {
      try { rmSync(home, { recursive: true, force: true }); } catch { /* best effort */ }
    }
  }, 60_000);
