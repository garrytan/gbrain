...
describe('buildCJKKeywordSql (#3986)', () => {
  ...
  test('terms without case variants use LIKE clauses, AND-joined, with explicit ESCAPE', () => {
  ...
  test('Hangul-only terms use LIKE while Latin terms keep ILIKE', () => {
    const hangul = buildCJKKeywordSql('앨범', ctx());
    const latin = buildCJKKeywordSql('Paseo', ctx());

    expect(hangul!.sql).toContain("cc.chunk_text LIKE $1 ESCAPE '\\'");
    expect(hangul!.sql).not.toContain('ILIKE $1');
    expect(latin!.sql).toContain("cc.chunk_text ILIKE $1 ESCAPE '\\'");
  });

  test('mixed queries choose LIKE or ILIKE independently for each term', () => {
    const built = buildCJKKeywordSql('앨범 Paseo 기획', ctx());
