...
describe('knobsHash determinism + cross-mode separation (CDX-4)', () => {
  ...
  test('KNOBS_HASH_VERSION constant exposed for migrations to bump on schema change', () => {
    ...
    // processes.
    // v0.42.63: bumped 12→13 to fold the resolved source-boost map (sb=)
    // — a search.source_boosts config change must not serve pre-change
    // rankings from cache.
...
describe('v0.42.3.0 — autocut knobs', () => {
  test('KNOBS_HASH_VERSION is 13 (12→13 source-boost fold, v0.42.63)', () => {
    expect(KNOBS_HASH_VERSION).toBe(13);
  });
