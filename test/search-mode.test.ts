...
describe('knobsHash determinism + cross-mode separation (CDX-4)', () => {
  ...
  test('KNOBS_HASH_VERSION constant exposed for migrations to bump on schema change', () => {
    ...
    // Bumped 15→16 to fold per-call page type filters (type=) into the cache
    // key, preventing filtered/unfiltered query-cache contamination.
    expect(KNOBS_HASH_VERSION).toBe(16);
  });

  test('page type filter participates in the knobs hash', () => {
    const knobs = resolveSearchMode({ mode: 'balanced' });
    const unfiltered = knobsHash(knobs);
    const person = knobsHash(knobs, { pageType: 'person' });
    const company = knobsHash(knobs, { pageType: 'company' });
...
describe('v0.42.3.0 — autocut knobs', () => {
  test('KNOBS_HASH_VERSION is 16 (15→16 page type filter fold)', () => {
    expect(KNOBS_HASH_VERSION).toBe(16);
  });
