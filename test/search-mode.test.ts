...
describe('knobsHash determinism + cross-mode separation (CDX-4)', () => {
  ...
  test('KNOBS_HASH_VERSION constant exposed for migrations to bump on schema change', () => {
    ...
    // must not be served to post-fix lookups. 11→12: autocut weak-top floor
    // (acmts=) — the floor shifts whether autocut cuts at all.
...
describe('v0.42.3.0 — autocut knobs', () => {
  test('KNOBS_HASH_VERSION is 12 (11→12 autocut weak-top floor; 10→11 was #1400 input_type)', () => {
    expect(KNOBS_HASH_VERSION).toBe(12);
  });
