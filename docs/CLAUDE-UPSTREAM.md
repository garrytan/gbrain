# CLAUDE-UPSTREAM.md — Upstream gbrain context (offloaded from CLAUDE.md)

> **Why this file exists**: fork's `CLAUDE.md` keeps the fork-specific
> rules tight (~150 lines) so it loads fast every Claude Code session.
> Everything below is upstream `garrytan/gbrain` reference content
> ("Architecture", "Key files", "Testing", "Build", "Pre-ship",
> "CHANGELOG voice", etc.) that auto-merges from upstream syncs but
> rarely changes fork behavior day-to-day.
>
> **For Claude**: read this when you need upstream gbrain context —
> e.g. before touching `src/`, before drafting a release flow, before
> shipping a PR. The fork CLAUDE.md links here.
>
> **Sync policy**: future `upstream/master` merges land any conflicts
> from this content **here**, NOT in CLAUDE.md. Keep fork CLAUDE.md
> auto-merge clean.
...
- `src/core/search/source-boost.ts` (v0.22.0) — Source-type boost map keyed by slug prefix. `DEFAULT_SOURCE_BOOSTS` (originals/ 1.5, concepts/ 1.3, writing/ 1.4, people/companies/deals/ 1.2, daily/ 0.8, media/x/ 0.7, openclaw-reference/chat/ 0.5) and `DEFAULT_HARD_EXCLUDES` (test/, archive/, attachments/, .raw/). `parseSourceBoostEnv` / `parseHardExcludesEnv` parse comma-separated `prefix:factor` pairs from `GBRAIN_SOURCE_BOOST` / `GBRAIN_SEARCH_EXCLUDE` env vars. `resolveBoostMap` and `resolveHardExcludes` merge defaults + env + caller `SearchOpts.exclude_slug_prefixes`/`include_slug_prefixes`.
...
- `test/e2e/search-swamp.test.ts` (v0.22.0) — reproduces the headline source-swamp case. Seeds a curated `originals/talks/article-outline-fat-code` page against two `openclaw-reference/chat/` pages stuffed with the same multi-word phrase. Asserts the article wins keyword AND vector ranking, that `detail=high` lets the chat swamp re-surface (temporal-query workflow preserved), and that `source_id` passes through the two-stage CTE intact. PGLite in-memory.
...
names a specific agent fork like `openclaw-reference`, that real name gets indexed by
...
**Specific rule: never say `openclaw-reference` in any CHANGELOG, README, doc, PR, or
commit message.** When the temptation is to illustrate with the real fork name:
- Reader-facing copy → `your OpenClaw` (covers openclaw-reference, Hermes, AlphaClaw,
...
`openclaw-reference` may appear in private artifacts (scratch plans under
...
specifics, scrub it as a small cleanup commit, the same way a stale openclaw-reference
