...
- `gbrain/mcp/rate-limit` exposes the bounded token-bucket `RateLimiter` used
  by the native HTTP transport, so embedding gateways preserve native rate
  behavior without mounting or copying that transport.
...
Use the exported native `RateLimiter` for that application rate boundary.
