...
6. **Import checkpoints name the import target, not the caller's CWD.**
   ...
   deciding whether to resume. Completed paths record progress, not the
   revision imported: the next run re-reads current files and compares their
   content hashes with the database, including paths listed as completed.
