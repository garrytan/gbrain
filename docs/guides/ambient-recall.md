...
- **Pull (works everywhere, including Codex + Postgres/Supabase):** the harness
  ...
  no hooks required. It is the primary path for Codex and opencode, which do
  not have GBrain lifecycle hooks wired.
- **Push (PGLite or Postgres + a hook-capable harness):** the bundled hook framework fires
