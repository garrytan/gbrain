...
| `claude-code` / `codex` / `opencode` / `traecli` | `gbrain hook user-prompt` | per-prompt injection inside a hook-capable harness; see "Harness hooks" below |
...
- **The feedback loop.** The serve logs each DELIVERED block's volunteered
  ...
  (`claude-code` by default; other adapters pass `--harness <host>`).
  `gbrain volunteer-context --stats` then shows
...
The hook lane rides the local serve process's narrow IPC socket. PGLite keeps
the socket in its data directory; Postgres uses a source- and harness-bound,
URL-hashed runtime directory below the gbrain home. Each MCP serve and hook
must share `GBRAIN_HARNESS`; active socket owners are never unlinked by a
second serve. TraeCLI is prompt-only: its non-Claude transcript path is ignored,
while Claude Code keeps strict transcript confinement and cross-turn dedupe.
Thin-client installs still have no
local engine or serve socket and therefore use pull-mode retrieval.
