...
- Current upstream baseline: `9bf96db807c2f050449142f2f0b05726f58e5054`
- Current branch commit at goal authoring time: `2889685a`
- Current upstream version after refresh: `0.42.51.0`
...
Deliver the docs-only consolidation PR for GBrain by executing slices S-01
through S-13 in order, with `CHANGELOG.md` as the current-capabilities baseline and with
...
- `docs/docs-consolidation/03-issue-breakdown.md` is the source plan.
- S-01 through S-13 are the ordered execution ledger.
- All issue slices are AFK.
- The first slice is S-01, changelog-to-current-capabilities baseline.
...
- `karpathy-agentic-guidelines-full`: use for S-01, S-12, S-13, and any delegated or
...
- Use `docs/docs-consolidation/03-issue-breakdown.md` as the local tracker
  truth unless upstream maintainers promote slices into repository issues.
...
- Operator feedback notes captured during planning
- Draft slices S-01 through S-13 as they become active
- `docs/INSTALL.md`
...
6. Execute final consistency pass.
   - S-13 maps PRD, changelog, source/docs, operator feedback, slices, and final
     diff.
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | S-01 | Changelog-to-current-capabilities baseline | agent_executable | Clean branch | Baseline artifact exists and maps current releases to doc obligations | implemented in `05-current-capabilities-ledger.md` |
| 2 | S-02 | Documentation inventory and status taxonomy | agent_executable | S-01 complete | Inventory/status taxonomy updated against baseline | implemented in `06-documentation-status-taxonomy.md` |
| 3 | S-03 | README router, current version, LLM entrypoints | agent_executable | S-01 and S-02 complete | README routes to canonical docs and current version | implemented in `README.md`; `llms-full.txt` regenerated |
| 4 | S-04 | Human Operational Center | agent_executable | S-01 and S-02 complete | `docs/INSTALL.md` carries the central human path | implemented in `docs/INSTALL.md`; `bun run build:llms` produced no generated diff |
| 5 | S-05 | Agent Operational Center | agent_executable | S-01, S-02, S-04 complete | `INSTALL_FOR_AGENTS.md` mirrors operational branches with safety gates | implemented in `INSTALL_FOR_AGENTS.md`; `llms.txt` and `llms-full.txt` regenerated |
| 6 | S-06 | Operating model and topology trees | agent_executable | S-01 and S-02 complete | Branchable choices exist and are linked | implemented in `docs/architecture/topologies.md`; linked from `README.md`, `docs/INSTALL.md`, and `INSTALL_FOR_AGENTS.md` |
| 7 | S-07 | Brain Repo Layout | agent_executable | S-01, S-02, S-06 complete | Layout/source/brain docs exist and are linked | implemented in `docs/architecture/brain-repo-layout.md`; linked from human, agent, and production paths |
| 8 | S-08 | Mode Selection Guide | agent_executable | S-01, S-04, S-06 complete | Guide covers current command and push-context paths | implemented in `docs/guides/mode-selection.md`; linked from human, agent, and production paths |
| 9 | S-09 | Production path and checklist | agent_executable | S-04, S-06, S-07, S-08 complete | Production branch/checklist exists inside human path | implemented in `docs/INSTALL.md`; production path remains centralized |
| 10 | S-10 | MCP/auth/remote/thin-client alignment | agent_executable | S-01, S-05, S-06, S-09 complete | MCP/auth docs align with current protocol truth | implemented in `docs/mcp/DEPLOY.md`, `SECURITY.md`, `docs/mcp/ALTERNATIVES.md`, client MCP recipes, and `docs/architecture/thin-client.md` |
| 11 | S-11 | Status labels | agent_executable | S-02 complete | Historical/design/deprecated/superseded docs labelled where needed | implemented in selected historical/design/proposal/incident/plan docs and tracked in `07-status-label-application.md` |
| 12 | S-12 | Brittle counts and generated-map claims | agent_executable | S-01, S-02, S-03 complete | Counts qualified/removed and generated maps handled | implemented in current docs, generator source, regenerated maps, and `08-brittle-counts-and-generated-maps.md` |
| 13 | S-13 | Final consistency pass | agent_executable | S-01 through S-12 complete | PRD/slices/changelog/operator feedback mapped to final diff | implemented in `09-final-consistency-report.md`; PR handoff prepared in `10-pr-proposal.md` |
