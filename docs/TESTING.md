...
- `test/import-abort-error.test.ts` — `runImport` preflight/argv failures print their reason to stderr and throw typed `ImportAbortError` instead of exiting the process; the calling process survives the abort. Covers the lock-admission refusal (managed-worktree marker named on stderr, underlying error kept as `cause`) and the symlinked root in a managed brain.
