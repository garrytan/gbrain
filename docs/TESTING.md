...
- `test/source-sync-strategy.serial.test.ts` — PGLite CLI contract for `sources add --strategy`, atomic set/unset while preserving unrelated config, stable list JSON strategy/origin fields, invalid-state repair hints, and exit codes.
- `test/sync-source-strategy.serial.test.ts` — real markdown/code imports proving invocation override → stored source row → markdown precedence, missing-source fail-fast, invalid fallback, JSON provenance, and `syncOneSource` re-resolution instead of stale caller config.
- `test/sync-strategy-cli.serial.test.ts`, `test/sync-cost-estimate.test.ts`, and `test/sync-cost-gate.serial.test.ts` — strict enum/`--all` rejection, help contract, classifier parity in preview, and explicit `--repo` + `--strategy` precedence.
