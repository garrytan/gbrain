...
- `test/generate-flag-registry.test.ts` — the flag-registry marker-segmentation rule: a `--flag` literal belongs to the command named in the enclosing `command === 'X'` head for every dispatch shape (plain, compound `&& args[0] === 'sub'`, multi-line compound). Also pins the comment-strip contract at every scan depth (a comment in a case block, an imported command module, or a one-level dep registers no flag), cross-checks `stripComments` against the TypeScript parser as ground truth so the strip can never delete a real flag literal, and asserts the hand-tuned `EXTRA_FLAGS` lane (`jobs --allow-protected`) survives regeneration.
