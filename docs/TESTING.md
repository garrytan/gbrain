...
- `test/mcp-governed-dispatch.test.ts` — public embedded-gateway seam: one immutable operation ceiling drives definitions + dispatch; source-array/operation mutation cannot widen or swap handlers; explicit deeply frozen config/auth/context with no ambient source/config fallback; native validation + `OperationError` mapping; unexpected handler/resolver errors scrubbed; dynamic resolver may narrow but cannot replace an advertised operation.
