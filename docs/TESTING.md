...
- `test/extract-atoms-provenance-links.test.ts` — PGLite behavioral coverage for page-derived atom identity, versioning, and provenance. Forces two distinct same-date source pages to return the exact same atom title and requires independently addressable atoms; forces a first provenance write failure in a non-default source and retries through normal production discovery to exactly one same-source edge; changes source content under an unchanged locator/title and requires a new version-bound atom, an untouched prior atom, and no warning loop. Also corrupts an existing exact-version binding and requires the canonical import path to fail closed without overwriting it.
