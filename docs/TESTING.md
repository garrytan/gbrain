...
- `test/embed-facts.test.ts` — `gbrain embed --stale --facts` (#4812) against PGLite with an injected embedder: every NULL-embedding fact is embedded and a re-run finds none; `--dry-run` counts without writing; expired rows are never selected; `sourceId` scopes count + selector; a throwing batch fails only its own rows while neighbours land; a short provider response is a failure with no misaligned write; an over-limit fact is reported, not sent; `updateFactEmbeddings` skips expired rows and rejects malformed input; and the literal `facts_pending` predicate from `migrate embeddings --status` counts exactly what `listFactsNeedingEmbedding` returns (cursor is exclusive, id-ordered).
- `test/embed-facts-audit.test.ts` — the CLI branch of `gbrain embed --stale --facts` against PGLite with a stubbed embed transport: the keyless-refusal predicate never swallows a `--facts` run; `--facts` without `--stale` and every chunk-only / opt-out flag (`--background`, `--no-embed`, `--pace`, `--slugs`, `--all`, `--priority`, `--catch-up`, `--include-null-signature`) exit 1 with the usage line before any DB work; `--source`/`--batch-size` fail closed (missing, flag-like, inline `=`, malformed or unknown source, `0`/`abc`/`5abc`/`1e3`, above the 500 cap) before the creds preflight; `--dry-run --json` is keyless-safe and maps onto `EmbedResult`; single-flight on the global facts lock (held → `lock_skipped` + exit 0, `--json` still prints one object; stolen mid-drain → the heartbeat aborts the drain as `lock_lost` in both the result and the JSON, rows left NULL); a throwing width probe and a throwing lock acquire both fail open; the `embed.facts` progress start event lands on stderr before the first provider call; a facts-column width that differs from the configured width exits 1 before the lock; an `embedding_disabled` config rejects a live run with `EmbeddingDisabledError`; plus `embedStaleFacts` loop edges (pre-aborted signal, batch clamp, failure-sample cap, progress arguments, NaN vector).
- `test/embed-chunk-flags.serial.test.ts` — chunk-path flag parity for `gbrain embed` without `--facts` (Proxy mock engine, dry-run, `maybeBackground` mocked to capture the payload): `--stale --source=other --dry-run` scopes every stale count to `other`; `--stale --source`, `--source --stale`, `--source=` and a malformed id exit 1 with zero engine calls; `--batch-size abc`/`=0` exit 1; the `--background` payload carries the resolved `sourceId`/`batchSize`.
- `test/stale-facts-bigint.test.ts` — `listFactsNeedingEmbedding` normalizes BIGSERIAL `fact_id` to a NUMBER on both engines (fake `sql`/`db` returning `42n`), so `--json` output never throws on a real Postgres int8 row.
...
- `test/e2e/facts-embedding-backfill-parity.test.ts` — `DATABASE_URL`-gated parity for the two facts-embedding engine methods behind `gbrain embed --stale --facts` (claimed by a `scripts/e2e-test-map.ts` row): `listFactsNeedingEmbedding` returns the same ordered NULL-row list on Postgres and PGLite (expired + audit rows excluded, `sourceId` scoping, exclusive `afterId` keyset paging whose two pages stitch identically), and `updateFactEmbeddings` fills only NULL rows, skips expired ones, stores a real vector read back by width, and writes nothing on an identical re-run — the fill-only path PGLite alone cannot prove.
