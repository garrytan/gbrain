...
- `test/extract-atoms-provenance-links.test.ts` — PGLite behavioral coverage for page-derived atom identity and provenance. Forces two distinct same-date source pages to return the exact same atom title (identical date, stem, and title hash), retries through the explicit page seam, and requires independently addressable atoms with correct `source_slug`, final `source_hash`, exact `source_quote`, and one matching provenance edge each. Also corrupts an existing atom binding and requires the canonical import path to fail closed without overwriting it.
