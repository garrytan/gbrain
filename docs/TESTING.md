...
- `test/build-schema-fresh.test.ts` — `src/core/schema-embedded.ts` freshness guard (mirrors the build-llms regen-drift guard): regenerates the embedded blob via the real `scripts/build-schema.sh` (`OUT_FILE` override) and asserts the committed file is byte-identical, plus determinism, the AUTO-GENERATED header, `SCHEMA_FILE`-override escaping (backtick/dollar), and verify-wiring (`check:schema-embedded` in the dispatcher, `verify` delegates to it, CI runs `verify`, the shell guard's own exit-0 path). Paired with the verify-pool `scripts/check-schema-embedded-fresh.sh`.
