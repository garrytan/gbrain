...
- `test/e2e/engine-parity.test.ts` — Postgres ↔ PGLite top-result and result-set parity for `searchKeyword` + `searchVector` (Postgres ranks pages then picks best chunk while PGLite returns chunks directly, so the source-boost behavior needs parity coverage), plus the entity-card boundary's exact visible fact count, explicit-open-thread filtering, privacy, and wire-date normalization. Skips without `DATABASE_URL`.
