...
- `test/import-connection-budget.serial.test.ts` — direct Postgres directory imports honor `GBRAIN_MAX_CONNECTIONS`: requested fan-out is clamped against the configured parent and two-connection child pools, a budget with no child capacity reuses the parent engine serially, unset budgets preserve legacy fan-out, and PGLite bypasses the Postgres clamp. Serial because the suite owns process-level pool environment and a module mock for child-engine connections.
