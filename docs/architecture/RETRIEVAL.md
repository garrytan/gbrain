...
The boost map resolves through a three-rung ladder: `DEFAULT_SOURCE_BOOSTS` ← the persistent `search.source_boosts` config key (JSON object, set via `gbrain config set search.source_boosts '{"resources/":0.8}'`) ← the `GBRAIN_SOURCE_BOOST` env var (env last — the emergency override that beats config). `resolveSourceBoostsForEngine()` is the one shared resolver; op-surface callers thread the resolved map to the engines via `SearchOpts.sourceBoosts`, and engines fall back to env-only `resolveBoostMap()` when nothing is threaded. The resolved map is serialized canonically (sorted keys, 4-decimal factors) into the query-cache knobs hash, so a boost change can never serve pre-change rankings from cache (one-time cache-miss spike after a change is expected). Temporal queries (`detail: 'high'`) bypass the boost so chat pages re-surface for time-sensitive lookups.
