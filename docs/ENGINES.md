...
1. **Auto-repair (default).** `PGLiteEngine.connect()` detects the abort,
   ...
   `gbrain doctor`. **The reset does not rebuild indexes.** pg_resetwal's
   documented hazard applies: a heap page flushed ahead of its index pages
   leaves a row that `gbrain get` and keyword search find but the HNSW vector
   index does not, so that page is unreachable by vector search at any
   candidate limit — and `gbrain doctor` cannot see it (its index probes are
   Postgres-only, and the data-dir check only runs when connect fails). Verify
   after a repair with `gbrain search diagnose "<phrase from a recently written
   page>" --target <its-slug>`: `vector rank=absent` alongside a keyword hit is
   the signature. Recover one page with `gbrain embed <slug>` (the re-embed
   writes fresh index entries); for many, `gbrain embed --all`, or rung 3.
   Safety bounds: repair only runs under a cleanly-acquired
