...
- **`zerank-2`** — SOTA multilingual cross-encoder reranker.
  ...
  Plus `zerank-1` and `zerank-1-small` for legacy or customer-managed needs.

Both land in cortex v0.35.0.0 behind the openai-compatible recipe path,
...
**Important:** `cortex config set embedding_model …` is NOT a live
...
from the **file plane** (`~/.cortex/config.json`) and the **env plane**
(`CORTEX_EMBEDDING_MODEL` / `CORTEX_EMBEDDING_DIMENSIONS`). The DB plane
...
```bash
export CORTEX_EMBEDDING_MODEL=zeroentropyai:zembed-1
export CORTEX_EMBEDDING_DIMENSIONS=2560
```
...
```bash
cortex embed --stale --limit 50    # smoke a small batch
...
```bash
cortex models doctor --json | jq '.probes[] | select(.touchpoint=="embedding_config")'
```

Expected: `status: "ok"`. Invalid dims (e.g. `1024`, `1536`, `3072`)
surface as `status: "config"` with a paste-ready
`cortex config set embedding_dimensions <one of 2560|1280|640|320|160|80|40>` fix hint.

## Reranker switch — zerank-2

The reranker is the bigger story: cortex had no cross-encoder reranker
...
before, just with an observable failure surfaced via `cortex doctor`.
...
small-tenant volume per the CLAUDE.md cost matrix.

### Verify

```bash
cortex models doctor --json | jq '.probes[] | select(.touchpoint=="reranker_config")'
```
...
`~/.cortex/audit/rerank-failures-YYYY-Www.jsonl` (ISO-week rotation).

`cortex doctor` reads the audit and surfaces:
...
| `reranker_health` doctor warns about auth | `ZEROENTROPY_API_KEY` not set or invalid | Re-export the env var; `cortex models doctor` to verify |
