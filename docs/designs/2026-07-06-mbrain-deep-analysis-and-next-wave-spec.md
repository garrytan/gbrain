...
- **Twenty-third follow-up wave: O5 shipped (operations.ts split).** The 7,897-line god-file drops to a 1,549-line registry assembler; all 88 inline ops moved verbatim into 12 domain modules following the existing `createXOperations()` factory pattern (largest: operations-put-page.ts 1,362 lines with the write-session/markdown-mirror machinery, operations-retrieval.ts 1,290 + operations-retrieval-params.ts 690; operations-shared.ts holds cross-module param helpers importing only operation-params.ts, so no cycles). Contract proof: golden manifest byte-identical (196 ops), registry order verified identical via runtime name-list diff, export surface preserved for cli/mcp/edge importers, O3's validation wrapper intact; new `test/operations-split-contract.test.ts` pins the 12-module layout and bans inline Operation literals in operations.ts. Full suite 3,763 pass / 0 fail at the agent tree.
- **Twenty-second follow-up wave: O1 first slice shipped (dialect seam, personal-memory family).** `src/core/engine-dialect.ts` (placeholder/jsonPlaceholder/supportsReturning/serverNow + `DialectQueryable` executor seam) and `src/core/shared-sql/personal-memory.ts` now carry the 8 parity-covered personal-memory methods once for both engines; 279 duplicated lines deleted (135 pg-engine-base / 144 sqlite-engine), engine methods become 1-3 line wrappers over a private `dialectQueryable` adapter. Row mapping deliberately stays engine-local (subtle mapper differences exist) so the slice is strictly behavior-preserving; parity seeds pass byte-identical, full suite 3666/0 at the agent tree. Accepted cosmetic SQL drift only (case of `excluded.`, `ON CONFLICT (id)` spacing). Next slices: note-structure (forces a transaction concept + boolean primitive), then memory-governance-lifecycle (12 methods, largest payoff); O7's single migration ladder waits until several families are through the seam.
- **Twenty-first follow-up wave: N-1 first deliverable shipped (rules draft, EV-2 gated).** `docs/MBRAIN_AGENT_RULES.next.md`: five-surface restructure (probe/evidence/discover/page-IO/sync + `remember` as the default governed write) at 477 words vs the live file's 803 body words (41% fewer words, ~39% fewer est. tokens). All load-bearing invariants preserved and pinned by ~26 literal anchors in `test/agent-rules-draft.test.ts`, which also fences: the draft carries DRAFT/EV-2/do-not-inject markings, the draft body stays under 65% of the live word count, the live `MBRAIN_AGENT_RULES.md` is untouched, and `setup-agent.ts` still references only the live filename (confirmed `loadAgentRules()` cannot pick up `.next.md`). Deliberately dropped: duplicate CLI command forms, the `admin_put_page` repair mention, one restated hash paragraph, preamble prose. Adoption = owner flips the file after the spec's EV-2 A/B; nothing changes at runtime until then.
- **Twentieth follow-up wave: O8 first slice shipped (service taxonomy ratchet).** `src/core/services/service-taxonomy.ts` assigns all service files (now 118) to ten domains with one-line boundary notes (retrieval 21, governance 21, maintenance 19, memory-inbox 16, orientation 15, infra 10, agent-session 8, notes 4, personal 4, tasks 2), and `test/service-taxonomy.test.ts` mirrors the parity-ratchet pattern: it `readdirSync`s the directory so every NEW service file fails CI until consciously assigned, stale entries fail, lists stay sorted/unique. Deliberately zero file moves/import churn (spec's physical `services/<domain>/` regrouping and near-duplicate builder folding remain the next slice); the domain names are the intended future subdirectories. Already caught three drive-by additions (procedural-memory, anticipation, memory-trust) during integration merges.
- **Nineteenth follow-up wave: O3 first slice shipped (param-validation seam).** Discovery: master already ran descriptor-level validation in both dispatch paths (PR 243's `validateOperationParams`/`dispatchOperation`), but inline in operations.ts with no coercion. The slice extracts the seam into `src/core/operation-params.ts` (ErrorCode/OperationError/ParamDef + validator, identical error wording, 17 TDD tests), keeps a thin registry wrapper layering the put_page route-first precondition, re-exports everything so 20+ importers are untouched, and adds opt-in `ParamDef.trim` with copy-on-write coercion (symbol-keyed params preserved). Zod rejected: not a dependency. Deliberately no op opts into trim yet (whitespace-sensitive params like put_page content/raw ingest bodies must never trim; handler-level requireString helpers stay as defense in depth); `trim` is excluded from manifest/MCP schema serialization so later per-op adoption cannot drift the golden manifest.
- **Eighteenth follow-up wave: N-8 shipped (trust tiers + injection lint).** `memory-trust-service.ts`: `SourceTrustTier` (`user_direct > verified_doc > extracted > imported > unattributed`) derived evidence-kind-first, then ref-shape heuristics; candidates take the strongest ref tier, capped for inferred/ambiguous extraction and imports, raised by verification, forced to `unattributed` when refuted. Disclosure-only wiring: `read_context` canonical reads and (when refs exist) `retrieve_context` candidates carry `source_trust_tier`; the answer trust footer gains per-read tiers; ranking untouched. `lintInstructionInjection` (EN+KO patterns: instruction overrides, tool-call blobs, base64 >= 200 chars) runs at the single candidate-creation choke point: flagged content is forced to `captured` with an appended review reason, and `preflightPromoteMemoryCandidate` re-lints stored content and denies promotion (`candidate_instruction_injection_flagged`) unless verified — auto-promote inherits the deny via preflight. One behavior change: flagged patch candidates also freeze at `captured`, adding explicit review friction to that path.
- **Seventeenth follow-up wave: N-7 shipped (clue-first retrieval, flag-gated).** Config `retrieval_clue_first_probe` (default off): on `broad_synthesis` queries whose resolved pool is deterministically weak (zero search-derived candidates, or top `scoreResolvedCandidate` < 24 with the usage bonus excluded for session stability), `retrieve_context` drafts one clue from already-retrieved compiled-truth snippets via an injectable `RetrieveContextClueGenerator` and runs it as one extra `candidateSearch` variant through the existing RRF fusion — no ranking-weight changes, no new ops, golden manifest untouched. Every use/skip is disclosed via result `warnings` (clue text capped at 600 chars, never surfaced as evidence); production wiring sets only the flag — no inline-LLM generator in the hot path (runner-backed generator is future wiring, matching the spec's judge-runner pattern).
- **Sixteenth follow-up wave: N-9 + EV-1c shipped together.** N-9 sleep-time anticipation: deterministic `buildAnticipationPack` (likely-next questions from open tasks, watched questions, recent attention, each with pre-resolved read selectors) as a new terminal dream family `anticipation` (order 17), config-gated `dream_anticipation_enabled` default off, persisted via the engine config store (`anticipation_pack_latest`), inspectable through admin-tier `get_anticipation_pack`. EV-1c: the live gold-set eval now also runs on PGLite (governed-probe on/off) plus deterministic stub-embedding configs (`stub-hash-1024` FNV-1a provider) on both engines — six byte-stable baselines with exact-match fences and a shared runner fixture so tests and the regen script cannot drift; real embedding-model baselines honestly skipped (require local model servers CI lacks).
- **Fifteenth follow-up wave: N-4 shipped (procedural memory loop).** Deterministic `detectProceduralPatterns` mines recurring action sequences from task attempts, decisions, and retrieval traces (no LLM); `propose_procedural_candidates` converts detections into governed Memory Inbox procedure candidates only with explicit `apply: true` (added to `CANONICAL_WRITE_MUTATING_OPERATIONS`), keeping the verify-then-promote pipeline authoritative. Both ops land at admin tier (default catalog holds at 103). The dream cycle's consolidation phase reports `procedural_pattern_proposals` counts but never creates candidates itself.
- **Fourteenth follow-up wave: N-3 shipped (core memory blocks).** Deterministic `buildCoreMemoryBlocks` compiles owner-profile, active-projects, and attention blocks from profile memory, open task threads + working sets, confirmed `read_context` traces (14d window), and watched-question deltas — hard-capped at 2,000 tokens by construction with a strict priority cutoff, every line source-attributed and labeled `not_answer_evidence`. Blocks are included in `plan_agent_session_activation` output, refreshed as a persisted snapshot during the dream-cycle consolidation phase, and inspectable via the admin-tier `get_core_memory_blocks` op (kept off the default catalog so the 103-tool surface holds).
- **Thirteenth follow-up wave: unreachable-intent routing closed, eval-gated.** Conservative deterministic signals in `buildAutoRouteInput`: slug-like known-subject/query tokens upgrade broad_synthesis-mapped scenarios to `precision_lookup` (slug becomes the route param, executing non-empty); episodic markers (EN+KO) + a personal_episode known-subject select `personal_episode_lookup`. Gold-set route-match 28/30 → 30/30 with byte-identical rows for the 28 prior passes; per_route now spans all 6 intents; baselines regenerated. The two spec canaries retired into real passing cases.
- **Twelfth follow-up wave: N-5 shipped (ungated lanes).** Deterministic memory-strength scoring from real traces — `strength = (2*reads + 3*answer_ready + probes − 4*conflicts) · exp(−days/half_life)` — distinguishing confirmed `read_context` reads from probe selections and reading conflicts from the trace verification codes; surfaces top/fading/never-used with true totals via `get_memory_strength_report` + a daily-report section + dream-cycle suggestions. Ranking consumption stays behind the EV-1b gate per RC-5.
- **Eleventh follow-up wave: N-6 shipped.** `trace_memory_contamination` (admin, read-only, deterministic) joins canonical handoffs, patch candidates citing `memory_candidate:<id>`, mutation-ledger events, and post-write retrieval traces into a contamination report with per-page re-verification next_actions; `refuted_contamination` daily-report section covers the trailing 7 days (top 10). Also: the stdio tools/list frame budget was raised once, principled (180,000 → 196,608 bytes) as the op catalog legitimately grew — the ≥16KB growth-headroom guard stays.
- **Tenth follow-up wave: O4 navigation consolidation.** 14 duplicative retrieval-navigation ops (6 route resolvers, 4 planning/scenario diagnostics, 4 workspace cards) demoted to explicit admin tier with one-line replacement-naming descriptions; default catalog 117→103 tools; default-tier tools/list frame −14.8KB (95.5→80.7KB); `select_retrieval_route` remains the consolidated selector. Pinned by test. Full lens-param folding and N-1's rules-file cut remain open (rules cut stays gated on an EV-2 A/B).
- **Ninth follow-up wave: P2 gate made real (EV-1b/EV-1c load-bearing parts).** The 30-case gold set now executes live in CI against a seeded corpus (20 pages + profile/episode/task fixtures): aggregate recall@10 = 1.0, MRR = 1.0, route-match 28/30 (two authored canaries pin the structurally-unreachable `precision_lookup`/`personal_episode_lookup` intents — the scenario classifier only emits 4 intents; noted as a routing follow-up). A regression-sensitivity test injects a broken ranking and requires collapse below 0.2. The self-comparing "non-regression" JSON is deleted; per-config baselines (sqlite governed-probe on/off, embeddings pinned to none for determinism) ship with `scripts/regen-retrieval-baselines.ts` and a live `--compare` test. EV-2b was verified already shipped upstream (G wired to the recall@10 proxy; J provably exceeds 0; `unavailable_without_gold` labeling). **The P3 gate precondition is now satisfied for the sqlite/no-embeddings config**; PG and ±embeddings baseline configs remain future work, as does surfacing the two unreachable intents.
- **Eighth follow-up wave: RC-3 shipped feature-detected.** SQLite vector search gains a sqlite-vec fast path (SQL-side `vec_distance_cosine` top-k with type/exclusion/temporal filters and a byte-length dimension guard) behind runtime feature detection: `tryLoadSqliteVec` on connect, JS brute-force fallback preserved byte-for-byte, `get_health.vector_backend` reports which path is live, `MBRAIN_CUSTOM_SQLITE` opts macOS into an extension-capable libsqlite3. Cross-backend ordering/score parity is tested where the extension loads (Linux CI) and skips honestly where it cannot.
- **Seventh follow-up wave: O2 personal + note-structure seeds.** `personal-memory` and `note-structure` parity families (profile/episode CRUD; manifest upsert/list/delete + section replace/get/list/delete) with pinned cross-engine snapshots; 16 more methods off the backlog — 42/168 covered.
- **Sixth follow-up wave (same day): O2 governance seed.** The `memory-governance-lifecycle` parity family now drives create → status events → staging → verification → promotion → canonical handoff record/completion through SQLite and PGLite with pinned identical snapshots — the first behavioral parity coverage of the 55-method governance store (12 methods off the backlog; 26/168 covered). Personal-memory and note-structure seeds remain open.
- **Fifth follow-up wave (same day): O4/N-1 first slice.** The deprecated 8-op context-atlas family is demoted to the admin tier with one-line deprecation descriptions — removed from the default agent catalog and recovering ~2KB of the nearly-exhausted tools/list frame headroom (now ~17KB above the 16KB floor). Full O4 consolidation (retrieval lens param, quartet collapsing) and the rules-file cut remain open.
