...
| `GBRAIN_EMBED_MAX_CHARS` | 8000 | Per-text character cap applied before any embedding request is sent. Local / self-hosted embedding servers (llama.cpp GGUF such as qwen3-embedding-4b) can time out or overflow their context window on a single very long input; lowering this bounds one input's length to what the operator's hardware handles, without forking the gateway. Complements `GBRAIN_EMBED_MAX_BATCH_TOKENS` above: that knob bounds a batch's shape, this one bounds a single text's length. Non-numeric or non-positive values fall back to 8000. Read once from the environment at process start, never at call time. |
