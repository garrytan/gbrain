...
| `GBRAIN_EMBED_MAX_BATCH_TOKENS` | (unset) | Token cap per embedding request for recipes that ship without one — ollama, lmstudio, and litellm declare `no_batch_cap` because real capacity depends on the operator's server. When set, chunk texts are pre-split into sub-batches within `cap × safety_factor` (default 0.8) tokens, estimated via the recipe's `chars_per_token` (default 4). A recipe-declared cap always wins; invalid values are ignored. Read once from the environment at process start, never at call time. Without it, `no_batch_cap` recipes still get a conservative 16-item sub-batch cap so the per-call embed timeout bounds a fixed amount of work. llama-server is the exception: its recipe ships no `no_batch_cap` and no recipe-level token cap — only `max_batch_items: 32` — so `GBRAIN_EMBED_MAX_BATCH_TOKENS` applies to it, and the 16-item fallback above does not. For CJK corpora, the default `chars_per_token: 4` estimate undercounts by roughly 4× (Chinese runs about 1 char/token), so budget `GBRAIN_EMBED_MAX_BATCH_TOKENS` at about a quarter of your llama-server `-b` budget rather than the full amount. Also note llama-server rejects any single input longer than `-ub` (`n_ubatch`) outright, and the token-budget pre-splitter never splits an individual text — raise `-ub` or lower your chunk size for overlong CJK chunks. |
