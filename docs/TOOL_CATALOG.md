...
| `get_image` | Return a page-owned image as a native MCP image content block. | read |  |  |
...
| `put_image` | Store a PNG, JPEG, or WebP page image through the configured files backend. | write |  | `mcp.publish_images` |
