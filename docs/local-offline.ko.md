# 레거시 로컬 SQLite 호환 가이드

이 문서는 레거시 local/offline SQLite 호환 프로필을 설명합니다.
새 로컬/관리형 설치에서는 Postgres + pgvector 런타임이 기본 target입니다.
일반적인 로컬 사용자는 README의 Postgres 런타임 quick start에서 시작하세요:

```bash
mbrain init --profile homebrew-postgres
```

이 레거시 SQLite 가이드는 **Supabase, OpenAI, Anthropic, Postgres 같은 필수
클라우드 또는 데이터베이스 서비스 없이** 의도적으로 MBrain을 사용하려는 경우를 다룹니다.
...
이 레거시 SQLite 가이드는 다음 조건에서만 적합합니다.

- 기존 `mbrain init --local` 설치와 호환해야 한다
- 로컬 Postgres 서버를 쓰지 않으려 한다
- Codex / Claude Code에서 로컬 MCP로 붙이고 싶다
- 기본 target Postgres 런타임 대신 SQLite를 명시적으로 쓰고 싶다

다음이 필요하면 target Postgres 경로가 더 적합합니다.

- 현재 기본 로컬/관리형 런타임
- pgvector 기반 semantic state
- 자동 assertion/canonical write 파이프라인
- 관리형 프로필에서 `mbrain files ...` 같은 클라우드 파일/스토리지 워크플로우
...
## 9. 에이전트 설정 미리보기, diff, 적용

MCP 등록과 행동 규칙 주입을 수동으로 하는 대신, managed setup 계획을 먼저
확인한 뒤 적용하세요:
...
```bash
mbrain setup-agent --preview    # 파일을 쓰지 않고 client/rules/hook 작업 미리보기
mbrain setup-agent --diff       # 파일을 쓰지 않고 redacted diff 표시
mbrain setup-agent --apply      # managed setup 작업을 명시적으로 적용
mbrain setup-agent              # --apply의 legacy compatibility alias
mbrain setup-agent --claude --apply     # Claude Code만
mbrain setup-agent --codex --apply      # Codex만
mbrain setup-agent --claude --scope local --apply  # Claude 프로젝트-local MCP 등록
mbrain setup-agent --skip-mcp --apply   # 규칙만 주입, MCP 등록 건너뜀
mbrain setup-agent --uninstall  # MBrain이 관리한 setup surface만 제거
mbrain setup-agent --print      # 파일 쓰기 대신 stdout 출력
mbrain setup-agent --json       # 기계 판독 가능 출력
```

파일을 바꾸기 전에는 `--preview`와 `--diff`를 먼저 사용하는 것을 권장합니다.
`--apply`가 권장되는 명시적 변경 경로이며, bare `mbrain setup-agent`는 기존
workflow를 위한 mutating compatibility alias로 남아 있습니다.
...
규칙은 `<!-- MBRAIN:RULES:START -->` / `<!-- MBRAIN:RULES:END -->` 마커로
감싸져 managed MBrain 블록 밖의 사용자 콘텐츠를 보존합니다.
`setup-agent`를 다시 실행하면 MBrain 섹션만 제자리에서 업데이트됩니다.
...
`doctor --agent --explain`은 Codex와 Claude가 동작하는 `mbrain serve`
명령에 등록되어 있는지, 등록 상태가 enabled/connected인지, 관리되는
prompt rules가 설치되어 있는지 확인합니다. read-only proof summary는
candidate, graph path, Dream output, raw episode가 기본적으로 hint-only임을
보고하고 deterministic proof status를 포함합니다. smoke 명령은 설치된 MCP
서버를 stdio로 실행하여 tool discovery, page lifecycle, search,
