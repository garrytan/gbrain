...
# Load GBRAIN_REMOTE_CLIENT_SECRET from the teammate's secret manager or
# protected process environment before running this block.
...
Because an environment-sourced client secret is deliberately not copied into
the config file, the teammate's secret manager or process launcher must inject
`GBRAIN_REMOTE_CLIENT_SECRET` for every later thin-client command and agent
session. The persisted config holds the issuer URL, MCP URL, and client id, not
that secret.

The OAuth client id/secret above configures the GBrain thin-client CLI. It does
not automatically configure every AI client's own MCP connector. Do not mint a
legacy bearer token for a scoped teammate: legacy bearer tokens carry broad
authority and do not preserve the `--source` and `--federated-read` isolation
established above.

For AI clients, do not point a thin-client home at local stdio `gbrain serve`.
That path is for same-machine brains.

- Clients that support OAuth client credentials may connect directly with a
  separately registered, least-privilege OAuth client.
- Bearer-only clients must not connect directly when source isolation is a
  requirement. Let the agent invoke the configured thin-client CLI commands,
  or wait for/use a connector that supports the scoped OAuth contract.
- Public authorization-code connectors require the authenticated consent
  policy described in [`SECURITY.md`](../../SECURITY.md); pre-registration
  alone is not an authorization boundary in the current server.
- Follow the matching page under [`docs/mcp/`](../mcp/) only after confirming
  that its authentication shape preserves the access policy for this client.
