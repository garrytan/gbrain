...
diagnostic (for example `owner_unavailable` or `commit_outcome_uncertain`)
drawn from a closed vocabulary; it is the recorded journal reason, while
`diagnostic.reason` is the derived health summary. Clients drop an unrecognized
`blocked_reason` rather than discard the receipt. Only `state` decides
commitment. On a terminal receipt it can only name retained recovery files
(`unexpected_staging_bytes`, `unexpected_file_bytes`) that need operator
reconciliation; the outcome is still final. `persistence.mode` distinguishes a
