...
- `open_threads` is fail-closed and capped at 3. A commitment or recent event
  is **not** automatically unresolved. Entries require either:
  - structured `frontmatter.open_threads[]` evidence (`text` plus
    `status: "open"` or `open: true`; remote callers additionally require
    `visibility: "world"`), or
  - an exact `[open-thread] ` prefix on a commitment fact or timeline summary.
    Fact visibility remains authoritative. Timeline entries are local-only
    unless they use the exact `[open-thread:world] ` prefix.
  Marker prefixes are stripped from the response. Unmarked facts/events and
  structured entries marked resolved are omitted.
- `active_fact_count` is the exact count of active facts visible to the caller
  (world-only for remote callers), not the number of rows fetched to assemble
  the card.
