---
conversation_id: 69e83cf7-2d58-83e8-b47a-b05a72ce161a
created: 2026-04-22
links:
- '[[chatgpt-clusters/business_startup]]'
message_count: 2
source: chatgpt-export
tags:
- chatgpt
- import
title: 8-Slide YC Pitch Deck
type: note
updated: 2026-04-22
---

# 8-Slide YC Pitch Deck

**Related:** [[business_startup]]

> Conversation ID: 69e83cf7-2d58-83e8-b47a-b05a72ce161a
> Created: 2026-04-22T03:14:12Z
> Updated: 2026-04-22T16:42:15Z
> Messages: 2

---

## User

whats the 8 slide yc puche deck

---

## Assistant

The **8-slide Y Combinator (YC) pitch deck** is a stripped-down format for startup fundraising. It focuses on clarity, speed, and showing traction over storytelling.

Here’s the standard structure:

---

### 1. **Problem**
- What specific problem are you solving?
- Who has this problem?
- Keep it concrete and relatable.

---

### 2. **Solution**
- What you built to solve it
- One sentence is ideal
- Demo or screenshot if possible

---

### 3. **Why Now**
- Why this didn’t work before
- What changed (tech, behavior, regulation, etc.)

---

### 4. **Market (TAM)**
- How big is the opportunity?
- Show a believable path to a large market
- Avoid inflated top-down numbers

---

### 5. **Product**
- How it works
- Key features or flow
- What makes it 10x better

---

### 6. **Traction**
- Proof people want it
- Revenue, growth, usage, retention
- This is one of the most important slides

---

### 7. **Team**
- Why you are the right people
- Relevant experience, speed, insight

---

### 8. **Ask / Vision**
- What you’re raising
- What milestones you’ll hit
- Long-term vision (brief, credible)

---

### Key YC principles
- Clarity > design
- Specifics > buzzwords
- Traction > ideas
- Shorter is better

If useful, I can tailor this into a concrete deck for your company with actual slide copy.


**Topic:** [[chatgpt-clusters/business_startup]]


## See Also
- [[chatgpt/chatgpt_marketing-highlights-pitch-deck]]
- [[chatgpt/chatgpt_pitch-deck-enhancement]]
- [[chatgpt/chatgpt_pitch-deck-idea]]
- [[chatgpt/chatgpt_pitch-deck-integration-plan]]
- [[chatgpt/chatgpt_pitch-deck-strategy]]
