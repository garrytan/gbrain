---
conversation_id: 92c9d23a-bb2c-4933-8f14-6a4cf25f05f7
created: 2023-05-04
links:
...
> Conversation ID: 92c9d23a-bb2c-4933-8f14-6a4cf25f05f7
> Created: 2023-05-04T17:19:45Z
> Updated: 2023-05-04T17:20:09Z
> Messages: 2
...
add a design option called auth

 class="mt-4 flex w-full justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2"

---

## Assistant

This is a Vue.js component template for a button that has different designs and sizes based on props passed to it. It also conditionally renders an SVG icon if a prop for a specific icon is provided.

The button has different CSS classes based on its props. Some of the props are:
- `design`: can be `primary`, `secondary`, `toolbar`, or `hero`.
- `size`: can be `xs`, `sm`, `base`, `md`, or `hero`.
- `icon`: can be `google`.

The CSS classes for the button are conditionally generated based on the props. For example, if `design` is `secondary`, the button will have a white background and a slate border. The `rounded` prop determines the button's border radius, and the `success` and `error` props determine the button's background color.
