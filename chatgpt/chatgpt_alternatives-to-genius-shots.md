...
conversation_id: 68aa4758-5618-8325-be07-73b4ad655858
message_count: 8
tags: [chatgpt, import, code, long-form]
---
# Alternatives to Genius Shots

> Conversation ID: 68aa4758-5618-8325-be07-73b4ad655858
> Created: 2025-08-23T22:58:39Z
> Updated: 2025-08-23T23:08:57Z
> Messages: 8
...
- **Consult your healthcare provider** immediately to evaluate:
  - Switching to **Zepbound** or **Wegovy** if available.
  - Considering other FDA-approved alternatives from the list above.
  - Temporary use of **phentermine** while sorting access to injectables.
- **Stay adaptive**: emerging options like retatrutide or CagriSema may be worth revisiting when available.
- **Continue supporting** any pharmacotherapy with mindful nutrition and resistance training to preserve lean mass and avoid dependency on medication alone.citeturn0news22turn0news23

---

Let me know if you'd like a deeper comparison (e.g., efficacy timelines, dosing schedules) or help exploring cost/access options.
...
export default function Dashboard() {
  return (
    <>
      <SessionWatch onExpire={() => {
        // Optional UX: toast “Session expired. Please sign in again.”
      }}/>
      {/* ...dashboard */}
    </>
  );
    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
    {
      global: {
        fetch: async (input, init) => {
          const token = await getToken({ template: "supabase" }); // refresh-capable
...
**Behavior:** if Clerk token refresh fails, calls in flight return **401** → caught upstream by `SessionWatch` path.
...
- **All protected API routes**:
  - Validate Clerk auth server-side.
  - If invalid → **return 401 JSON** `{ error: "SESSION_EXPIRED" }`.
  - Frontend intercepts 401, triggers `SessionWatch` path.
...
**Global fetch interceptor (browser)**
```ts
// lib/net/fetch-guard.ts
export async function guardedFetch(input: RequestInfo, init?: RequestInit) {
  const res = await fetch(input, init);
  if (res.status === 401) {
    // Bubble to SessionWatch by throwing a typed error
    const body = await res.json().catch(() => ({}));
    const err: any = new Error("Session expired");
    err.code = body?.error ?? "SESSION_EXPIRED";
    throw err;
  }
  return res;
}
...
## 5) Draft Autosave (lossless UX)

**`lib/drafts/index.ts`**
```ts
export async function saveDraftSnapshot() {
  // Implement per-screen; default fallback:
  const key = `draft:${location.pathname}`;
  const payload = { ts: Date.now(), state: window.__APP_UI_STATE__ ?? null };
  localStorage.setItem(key, JSON.stringify(payload));
}

export function restoreDraftSnapshot(pathname: string) {
...
export function clearDraftSnapshot(pathname: string) {
  localStorage.removeItem(`draft:${pathname}`);
}
```

**On successful re-auth:**
- In `app/sign-in/complete` (post-auth callback), read `returnTo`, `restoreDraftSnapshot(returnTo)`, rehydrate the form/state, then `clearDraftSnapshot(returnTo)`.

---

## 6) Sign-in page behavior

- Keep it **one-click**: default to last provider used.
- Show small, neutral banner if `?reason=expired`: “Session expired. Please sign in again.”

**`app/sign-in/page.tsx`**
```tsx
import { SignIn } from "@clerk/nextjs";

export default function Page({ searchParams }: { searchParams: { returnTo?: string; reason?: string } }) {
  return (
    <div className="mx-auto max-w-sm py-10">
      {searchParams?.reason === "expired" ? (
        <p className="mb-3 text-sm text-muted-foreground">Session expired. Please sign in again.</p>
      ) : null}
      <SignIn redirectUrl={searchParams?.returnTo ?? "/dashboard"} />
    </div>
  );
}
```
...
## 7) SSR data loaders (fail fast, redirect)

For server components that fetch Supabase data:
- If Clerk `auth()` missing → redirect to `/sign-in?returnTo=...`.
- Don’t attempt DB queries with stale/anonymous context.
...
Track:
- `auth.silent_refresh.success` / `auth.silent_refresh.fail`
- `auth.session.expired.redirect`
- Mean time from 401 → re-authed page render.
- Draft-restore rate.

---

## 9) Edge cases to handle
- **Tab sleep/wake** (macOS App Nap): first request after long idle → 401 → guard handles it.
- **Token clock skew**: always trust server; do not client-extend.
- **Background mutations**: queue writes; if 401, persist the mutation payload to localStorage, then replay after re-auth.

---

## 10) Acceptance tests (Playwright)

**`tests/auth-expiry.spec.ts`**
```ts
import { test, expect } from "@playwright/test";

test("expired session redirects, preserves returnTo, restores draft", async ({ page }) => {
  // 1) Login helper
  ...
  await page.getByLabel("Title").fill("My Smart Link");
  // simulate expiry by clearing auth cookie/storage used by Clerk in test env
  await page.context().clearCookies(); // or a helper that invalidates session in mock

  // 3) Next action should cause 401 → redirect to sign-in
  await page.getByRole("button", { name: "Save" }).click();
  await page.waitForURL(/\/sign-in\?returnTo=\/dashboard\/links\/new/);

  // 4) Re-login
  // (implement test login helper)
  await page.goto("/dashboard/links/new");

  // 5) Draft restored
  await expect(page.getByLabel("Title")).toHaveValue("My Smart Link");
});
```
...
- [ ] `middleware.ts` + `(protected)/layout.tsx` guards in place.
- [ ] `SessionWatch` mounted on all protected screens.
- [ ] Supabase client injects **fresh Clerk token** per request.
- [ ] API returns 401 JSON on invalid session; front-end intercepts.
- [ ] Draft autosave + restore wired to creation/edit screens.
- [ ] Sign-in honors `returnTo` and shows gentle “expired” banner.
