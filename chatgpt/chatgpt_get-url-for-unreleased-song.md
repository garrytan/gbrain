---
title: "Get URL for Unreleased Song"
type: note
created: 2024-05-31
updated: 2024-05-31
source: chatgpt-export
conversation_id: 11dc188b-21ae-405b-b730-d4c0e0a227c4
message_count: 2
tags: [chatgpt, import]
---
# Get URL for Unreleased Song

> Conversation ID: 11dc188b-21ae-405b-b730-d4c0e0a227c4
> Created: 2024-05-31T18:19:30Z
> Updated: 2024-05-31T18:19:35Z
> Messages: 2

---

## User

i have a new song on spotify. i know the uri. the song is not out yet. how can i get the url

---

## Assistant

To get the URL for a Spotify song that is not yet released, follow these steps:

1. **Open Spotify**: Go to the Spotify app on your computer or web player.
2. **Find the Song**: Locate the song using its URI.
3. **Get the URL**:
   - Right-click (or click the three dots) on the song.
   - Select "Share".
   - Choose "Copy Song Link".

This link will be a URL that you can share, even though the song is not released yet. It will direct people to the song's page on Spotify once it's live.


**Topic:** [[chatgpt-clusters/jovie_music]]
