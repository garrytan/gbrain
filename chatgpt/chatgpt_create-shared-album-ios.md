---
conversation_id: 683171c9-c94c-8007-9c9b-04517816737b
created: 2025-05-24
links:
- '[[chatgpt-clusters/jovie_music_industry]]'
message_count: 2
source: chatgpt-export
tags:
- chatgpt
- import
title: Create Shared Album iOS
type: note
updated: 2025-05-24
---

# Create Shared Album iOS

**Related:** [[jovie_music]]

> Conversation ID: 683171c9-c94c-8007-9c9b-04517816737b
> Created: 2025-05-24T07:14:18Z
> Updated: 2025-05-24T07:14:54Z
> Messages: 2

---

## User

How the fuck do you make a shared album on iOS in the newest version?

---

## Assistant

Mr. White, here’s how to create a Shared Album on the latest iOS:

1. **Enable Shared Albums**  
   Go to Settings > [your name] > iCloud > Photos, then toggle on Shared Albums. citeturn1view0

2. **Open Photos and find Shared Albums**  
   In Photos, tap the Albums tab, scroll down to Shared Albums, then tap Shared Albums > Create. citeturn3view0

3. **Name, add and share**  
   Enter an album name, pick the photos/videos you want, tap Next (or Create), choose contacts to invite, then tap Done. citeturn3view0


**Topic:** [[chatgpt-clusters/jovie_music]]


## See Also
- [[chatgpt/chatgpt_album-art-in-streaming-era]]
- [[chatgpt/chatgpt_album-art-request]]
- [[chatgpt/chatgpt_album-sales-2024-12-artists]]
- [[chatgpt/chatgpt_album-vs-ep-disadvantages]]
- [[chatgpt/chatgpt_allowing-actions-to-create-prs]]
