...
### Footer toggle (pseudo / React)
```tsx
function ThemeToggle() {
  const get = () => localStorage.getItem('tw_theme') || 'auto';
  ...
  return (
    <nav aria-label="Appearance" className="flex items-center gap-3">
      <span className="text-sm opacity-80">Appearance:</span>
      {['auto','light','dark'].map(opt => (
        <button
          key={opt}
          onClick={() => setMode(opt)}
          aria-pressed={mode===opt}
          className={`text-sm underline-offset-4 hover:underline ${mode===opt?'font-semibold':''}`}
        >
...
```css
:root{
  --bg:#ffffff; --bg-elevated:#f7f7f8;
  --fg:#0b0b0c; --muted:#6b7280;
  --border:#e5e7eb;
  --accent:#111827; --accent-contrast:#ffffff; /* elegant, brand-agnostic */
  --success:#16a34a; --warning:#f59e0b; --error:#ef4444;
  --ring: 2px solid #11182733;
}
[data-theme="dark"]{
  --bg:#0b0c0f; --bg-elevated:#14161a;
  --fg:#e6e7ea; --muted:#9aa0aa;
  --border:#2a2e36;
  --accent:#e6e7ea; --accent-contrast:#0b0c0f;
  --success:#22c55e; --warning:#fbbf24; --error:#f87171;
  --ring: 2px solid #e6e7ea33;
}
```

### Tailwind bridge (optional)
- In `tailwind.config.js`, extend colors using CSS vars:
```js
theme: {
  extend: {
    colors: {
...
## Design Specs (make it incredible)
- **Backgrounds**
  - Light: paper-white base, subtle elevated cards.
  - Dark: near-black (`#0b0c0f`) with slightly lifted surfaces; no washed gray.
- **Typography**
  - Keep current type scale; in dark mode, **increase line-height by +0.025em** for readability.
- **Cards/Sections**
  - Use **soft shadows** in light; **subtle borders** in dark (avoid lifted gray blobs).
- **Buttons**
  - Default: neutral monochrome; hover = subtle elevation; active = compress.
  - Focus ring uses `--ring`.
- **Links**
  - Underline on hover only; underline offset 4px; maintain contrast WCAG AA.
- **Media**
  - If images look too bright in dark, allow an optional `.invert-on-dark` utility using CSS `filter: brightness(0.9) contrast(1.05)` on `[data-theme=dark] .invert-on-dark`.
- **Icons**
  - Use current monochrome SVGs; they’ll inherit `currentColor`.

## Accessibility & QA Checklist
- [ ] All text contrast meets **WCAG AA** (use axe or Lighthouse).
- [ ] Focus states visible on all interactive elements.
- [ ] Toggle is keyboard navigable and announced (aria-label, `aria-pressed`).
- [ ] `prefers-reduced-motion` respected; no large parallax in dark only.
- [ ] No FOUC: verify correct theme at first paint (throttle CPU/network to test).

## Analytics
- [ ] Fire `theme_initial_resolved` on first load with `resolved: auto->dark|light`.
- [ ] Fire `theme_change` when user toggles (include from→to, pathname).

## Testing
- **Browsers**: Latest Chrome, Safari, Firefox; iOS Safari; Android Chrome.
- **Emulations**: System dark/light, reduced motion, high contrast mode.
- **Pages**: `/`, `/music`, `/shows`, `/contact`, `/tim-white-bio`. citeturn0search0turn0search1turn0search5turn0search4turn0search6
- **Perf**: Lighthouse dark vs light; CLS remains stable on theme change.

## Rollout
- **Branch**: `feat/dark-mode`
- **Flag**: none (ship whole)
- **Release notes**: “New: Dark Mode with Auto/Light/Dark toggle (footer).”

## Acceptance Criteria
- System dark users see dark theme **on first paint** with no flicker.
- Footer shows **Auto/Light/Dark**; selection persists across pages and reloads.
- All primary components visually polished in both modes; no low-contrast text.
- Lighthouse Accessibility ≥ 95 in both themes on `/` and `/music`.
