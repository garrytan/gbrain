...
> **Date:** 2025-08-03T19:37:39Z
> **Messages:** 6
> **Topic:** business-startup

## Question

To MVP out our Jovi Lincoln Bioproduct, I think we should use Clerk's waitlist feature to launch immediately with our landing page in place, but just instead of, but on production, hiding our artist search and instead having just a get on the waitlist button. And when you click that, it takes you to a waitlist page, it can be like slash waitlist or something, and we embed the waitlist widget from Clerk, and then if it's not a production environment, it doesn't show the waitlist widget and instead shows the artist search. And then we can go ahead and we can deploy that version to, we can go ahead and push that code and then get that going through our CI and appearing on our production server and then once that goes to production because it's passed all the checks and everything, we can iterate on the landing page a little bit, get our landing page nice and clean, and then we'll have basically a live landing page and we'll have live example profiles and then we can go back in and we can 
...
- **Growth**
  - Waitlist → email onboa

*(Answer truncated — 4403 chars total)*
