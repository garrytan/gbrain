...
4) **Artist profiles (local)**
   - Locate profile route(s). Ensure they **hydrate without client errors**:
     ...
     - If Spotify/remote fetches are part 

*(Answer truncated — 5850 chars total)*
          ...
          {artists.map(a => (
            <li key={a.id} role="listitem" className="shrink-0">
              <a href={`/${a.handle}`} aria-label={`Open ${a.name} profile`}
                 className="block rounded-full ring-1 ring-black/5 outline-none foc
...
*(Answer truncated — 4257 chars total)*
...
- You keep **RLS** everywhere, can run on **Edge**, and reduce one dependency—just less DX 
...
*(Answer truncated — 4120 chars total)*
