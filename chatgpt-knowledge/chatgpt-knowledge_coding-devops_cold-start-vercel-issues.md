...
If anything in the request path (middleware, edge route handlers, instrumentation) runs on Edge, **Node-only APIs** or a

*(Answer truncated — 5957 chars total)*
...
- **App Router**: a layout or component used 

*(Answer truncated — 4636 chars total)*
...
**Tell-tale sign**
- The 500 body is very small (your 0.3 KB fits), beca

*(Answer truncated — 4486 chars total)*
...
If you paste either (a) the **response headers** for the `/` request (especially `x-vercel-id`, `x-vercel-cache`) or (b) the **s
...
If you paste the **Response Headers** from your browser for the failing `/` request (the part that contains `x-vercel-id`, `x-vercel-cache`, and potentially `x-vercel-mitigated`), I’ll tell you immediately wh
