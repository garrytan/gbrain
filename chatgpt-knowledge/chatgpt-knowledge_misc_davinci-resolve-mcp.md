...
1. **Observer/control UI**  
   It shows a live feed of browser commands, command status, refs, and overlays on the page. After a `$B snapshot`, it shows clickable/referenceable `@ref` elements, so Claude can say things like `$B cli

*(Answer truncated — 4429 chars total)*
