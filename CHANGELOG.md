...
- Fixed `gbrain upgrade` for canonical `bun install -g github:garrytan/gbrain` installs by running `bun update gbrain` from Bun's global install root instead of the user's shell cwd. `BUN_INSTALL` overrides are honored. Contributed by @clarajohan (#1029).
