...
**Explicit `expansion_model` can now reuse chat-capable providers without pretending every chat recipe is a first-class expansion provider.**

This closes a real configuration gap in the recipe layer. The runtime expansion path for openai-compatible providers was already just `languageModel(modelId)`, but the capability gate required a dedicated `touchpoints.expansion` declaration. Result: advanced users pointing `expansion_model` at a chat-capable OpenAI-compatible gateway got silently downgraded to "no expansion" even though the transport could have served the request. New behavior is intentionally narrow: if a recipe declares a dedicated `expansion` touchpoint, nothing changes; if it does not, but it does declare `chat`, an explicitly configured `expansion_model=provider:model` may reuse that chat-capable provider. Provider discovery surfaces this as `EXPAND=via-chat` so the dedicated-vs-fallback distinction stays visible.

### Itemized changes

- `src/core/ai/model-resolver.ts`: new `allowsExpansionViaChat()` guard. `assertTouchpoint()` now permits `touchpoint='expansion'` on chat-only providers only when the model came from explicit gateway config (`extendedModels`) — default recipe semantics stay intact.
- `src/core/ai/gateway.ts`: `isAvailable('expansion')` now recognizes the same explicit-config fallback path. Dedicated expansion recipes still win; chat reuse is the advanced-user escape hatch rather than the default support matrix.
- `src/commands/providers.ts`: `gbrain providers list` / `explain` surface chat-backed expansion fallback as `via-chat` instead of falsely reporting "no expansion." Help text documents the meaning of the new label.
- `test/ai/gateway.test.ts`: regression coverage for `isAvailable('expansion')` and `assertTouchpoint(..., 'expansion', ...)` on explicitly configured chat-only OpenAI-compatible providers.
- `test/ai/providers-list.test.ts`: CLI presentation pin for `EXPAND=via-chat` on chat-only providers.
