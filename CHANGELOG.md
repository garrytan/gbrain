...
- Tier-aware response filtering for read paths via a per-op shape map (`OP_FILTER_SHAPE`) so the filter switches on declared shape rather than guessing: `get_page` (single-page) throws `page_not_found` for hidden slugs and filters the ambiguous-slug `candidates` list; `list_pages`, `search`, `query`, `get_recent_salience` (page-array) drop rows outside the visible prefix set; `find_orphans` (orphans-wrapper) filters the inner `orphans` array and recomputes `total_orphans`/`excluded`; `resolve_slugs` (slug-string-array) filters bare-string slugs. `get_chunks` is gated input-side in its handler (chunks rows have no slug field, so a response-side filter is structurally inert). Defaults derived from `templates/ACCESS_POLICY.md.template` (Work: `people/`, `companies/`, `deals/`, `projects/`, `tasks/`, `calendar/` and the `wiki/*` mirrors; Family: `logistics/`, `meetings/`, `calendar/`, `scheduling/`). Operators wire a custom `TierPrefixMap` through `FilterContext.prefixes`.
- `gbrain serve --enforce-access-tiers`: when set, the MCP handler returns `insufficient_tier` for callers whose tier does not satisfy `op.tier`. Default off — when off, the handler logs the would-be decision to `mcp_request_log` (status `warn`, `latency_ms` records dispatch-side overhead before the handler runs), broadcasts an SSE event with `would_reject: true`, and proceeds, so operators can query/dashboard audit-only signal before flipping enforcement on. Stdio MCP is local-only and bypasses tier filtering by design; tier checks apply to HTTP MCP only.
...
- Tests: `test/access-tier.test.ts` covers the primitive (parse/coerce/imply, prototype-pollution regression, all four filter shapes against real return types from `get_chunks`/`find_orphans`/`resolve_slugs`/`get_page` ambiguous, plus negative coverage that mutating ops are pass-through). `test/e2e/access-tier-enforce.test.ts` boots `serve --http --enforce-access-tiers` end-to-end and asserts a Work-tier OAuth client receives `page_not_found` on a `personal/` slug.
