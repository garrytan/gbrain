...
**Known limitations (v0.31.x follow-up):** `search`, `query`, `list_pages`, and `resolve_slugs` use a SQL `LIMIT N` followed by post-filter, so a non-Full caller's result-count and rank-order encode hidden-page distribution within the top-N (the response-filter strips hidden slugs but the rank within visible space was computed against the whole brain). Closing this requires SQL push-down of the tier prefix list into the engine's search/list/resolve queries; tracked for the next access-tier wave. The `get_page` fuzzy path is already pre-filtered in this PR (the most exploitable case — a single fuzzy probe disclosed ambiguity over hidden candidates).
- `gbrain auth register-client --tier <Full|Work|Family|None>` (default Full); `gbrain auth set-tier <client_id> <tier>` to retag existing clients (refuses soft-deleted clients). `gbrain auth list-clients` shows OAuth clients with their `access_tier` and grant times. INNER JOIN on `verifyAccessToken` with `c.deleted_at IS NULL` so an orphan or soft-deleted client's surviving token cannot fall open to Full.
