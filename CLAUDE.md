...
- `src/core/{pglite-engine,postgres-engine}.ts` getHealth soft-delete / stale semantics — health now scores only live pages (`deleted_at IS NULL`) and live-page links. Soft-deleted rows no longer inflate `page_count`, `orphan_pages`, missing embeddings or link-density scoring, and links from soft-deleted pages do not count as live graph connectivity. `stale_pages` uses actual past timeline event dates (`timeline_entries.date <= CURRENT_DATE`) against `effective_date`/`updated_at`, not the extraction row's `created_at`, so old imported timelines and future scheduled events do not create false stale health. Pinned by `test/brain-score-breakdown.test.ts` health regression cases.
