...
- `src/commands/repair-default-source.ts` (v0.30.3) — `gbrain repair-default-source [--dry-run] [--json]`: relocates pages misfiled in `source_id='default'` due to the v0.18-v0.22 multi-source sync regression (`importFromContent` didn't thread sourceId to `tx.putPage`, so syncs leaked every page to default even when the rightful `(source, slug)` row existed). For each leaked row, walks configured sources' `local_path` for a matching `<slug>.md` (or code-extension): unique match + no conflict → UPDATE `source_id`; unique match + conflict → DELETE the leaked duplicate; ambiguous/orphan → leave alone. Idempotent.
- `src/core/engine.ts` (v0.30.3) — added `SourceOpts { sourceId? }` and `LinkSourceOpts { fromSourceId?, toSourceId?, originSourceId? }`. 13 method signatures gained optional source opts (putPage, deletePage, upsertChunks, getChunks, deleteChunks, addLink, removeLink, getLinks, getBacklinks, addTag, removeTag, getTags, addTimelineEntry, createVersion, getVersions, revertToVersion). When `opts.sourceId` is omitted, writes default to `'default'` and reads scan-all (back-compat); when provided, every per-slug query scopes by `(source_id, slug)`. Closes the multi-source sync regression where `importFromContent` silently leaked pages to default and downstream scalar subqueries (e.g. `addLink`'s `(SELECT id FROM pages WHERE slug=$1)`) exploded mid-transaction with "more than one row returned by a subquery used as an expression".
