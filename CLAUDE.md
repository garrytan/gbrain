...
- `src/core/embedding.ts` — OpenAI embeddings, batch, retry, backoff. Exports `resolveEmbeddingModel()` which reads `GBRAIN_EMBEDDING_MODEL` env var (allowed: `text-embedding-3-large` default, `text-embedding-3-small` 6.5× cheaper). Callers at `src/commands/embed.ts` and `src/core/import-file.ts` stamp the resolved model on `ChunkInput.model` so `content_chunks.model` reflects reality on mixed-model brains.
...
- `scripts/filter-gmail-mbox.ts` — Gmail mbox pre-filter for historical backfills. Drops noreply/List-Unsubscribe/calendar-invite noise (60-80% volume reduction), strips quoted replies, writes one markdown file per kept message under `YYYY/MM/{slug}-{sha1-8}.md` with email frontmatter. Usage: `bun scripts/filter-gmail-mbox.ts <input.mbox> <output-dir> [--keep d1,d2]`. Paired with `GBRAIN_EMBEDDING_MODEL=text-embedding-3-small` to bring a 20-year archive from ~$25 down to under $5.
...
`test/embedding.test.ts` (resolveEmbeddingModel: default, explicit -small/-large, rejection of unsupported values, error-message shape),
`test/filter-gmail-mbox.test.ts` (Gmail mbox splitter, MIME parser, email-address extract, noise filters, QP/base64 body decode, quoted-reply strip, slug sanitize, markdown emission).
