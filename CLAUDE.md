...
- `src/commands/frontmatter.ts` (v0.32+) — `gbrain frontmatter {validate, audit, generate, install-hook, slug, abi-version}`. The `slug <rel-path>` and `abi-version` subcommands (added 2026-05-15) are the **external producer contract**: producers that write markdown into a gbrain-backed vault (webex-digest-package, ai-apple-transcripts) shell out here via `execFile`-style invocation to source slug computation from one place instead of porting `slugifyPath()`. **CRITICAL** — if you change `slugifyPath()` or `slugifySegment()` in `src/core/sync.ts` in a way that alters output for any input that previously produced a different result, bump the ABI integer (the `'1\n'` literal in the `abi-version` case in this file) so producers' startup assertions fail loudly rather than silently miscomputing slugs and tripping `SLUG_MISMATCH` at next sync. Producers currently in production: `~/git/webex-digest-package` (Python, shells out from `customized/scripts/stamp_frontmatter.py`); `~/git/ai-apple-transcripts` (TypeScript, shells out from `src/markdown-writer/filename.ts` and asserts ABI in `src/cli/gbrain-abi-check.ts`). User-facing producer integration guide at `docs/guides/producer-tools.md`. Design spec (webex side): `~/git/webex-digest-package/docs/superpowers/specs/2026-05-15-gbrain-slug-centralization-design.md`.
