...
| `GBRAIN_SYNC_RECONCILE_FORCE` | unset | Set to `1` to override the full-sync reconcile's mass-delete circuit breaker (refuses an automatic sweep of >=10 pages AND >20% of a source's file-backed pages — the signature of import-provenance loss, not a real cleanup). Also documented in `docs/architecture/KEY_FILES.md` (sync.ts entry). The reconcile spares pages whose latest `gbrain import <dir>` root is outside the repo (ingest_log provenance guard); `SyncResult.reconcileSkipped` reports when it skipped fail-safe. |
