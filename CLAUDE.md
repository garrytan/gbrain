...
**v0.42.63.0 knobs_hash v=12 → v=13.** Folds the resolved source-boost map
(defaults ← `search.source_boosts` config ← `GBRAIN_SOURCE_BOOST` env) into
the cache key so a boost change can't serve pre-change rankings. Per-call
result-set filters (`frontmatter_filter`, `types[]`, `since`/`until`) skip
the cache instead of hashing (`hasCacheSkippingFilters` in hybrid.ts).
